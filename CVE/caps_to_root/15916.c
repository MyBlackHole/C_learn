...
int main(int argc, char *argv[]) {
  ...
  if (!proto_tab || !commit_creds || !prepare_kernel_cred || !pn_ops ||
      !pn_ioctl) {
  ...
  map = mmap((void *)landing, 0x10000, PROT_READ | PROT_WRITE | PROT_EXEC,
  ...
  /* Map it */
  map = mmap((void *)0x30000000, 0x2000000, PROT_READ | PROT_WRITE | PROT_EXEC,
             MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, 0, 0);

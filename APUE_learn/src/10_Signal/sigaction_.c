...
void add_sigaction(int signo, sigset_t *set, int no_deffer, int restart,
                   Hanlder_Func handler) {
  sigset_t empty_set;
...
void test_sigaction() {
  ...
  //******** 添加信号处理程序 ********//
  add_sigaction(SIGINT, NULL, 1, 1,
                sig_print); // 几种组合：是否 no_deffer，是否 restart

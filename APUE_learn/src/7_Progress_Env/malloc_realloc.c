...
void test_malloc_realloc() {
  ...
  ptr4 =
      My_realloc(ptr3, 1000); // 扩大，ptr1！=ptr3(ptr1 释放掉，ptr3 重新分配)
  ptr5 = My_realloc(NULL, 100); //等价于 malloc(100)
  //************* 释放内存 **************//
  free(ptr5); // ok ，因为是新分配的
  free(ptr4);
  free(ptr2); // ok， 因为是新分配的
  //    free(ptr1); // 错误：因为 ptr1=ptr3，而 ptr3指向的内存在
  //    My_realloc(ptr3,1000) 时被释放 free(ptr3);// 错误：因为 ptr1=ptr3，而
  //    ptr3指向的内存在 My_realloc(ptr3,1000) 时被释放

...
int My_pthread_mutex_timedlock(pthread_mutex_t *mutex,
  ...
  if (0 != result) {
    printf(
        "thread(0x%x) call thread_mutex_timedlock(%p,%p) failed,because %s\n",
 ...
 * \param arg : 参数，这里是个整数，如果 arg=0 则不加锁；如果 arg=1
 * 则加锁；如果arg=2 则使用 try_lock，如果 arg=3, 则使用 timedlock \return
 * :返回读到的 shared_int 值（如果加锁失败则返回 -1）
 */
static int thread_func(int arg) {
  //********** 选择四种加锁行为 *************//
  switch (arg) {
  case 0: // 不加锁
    break;

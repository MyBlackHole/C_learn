...
int My_pthread_create(pthread_t *tidp, const pthread_attr_t *attr,
  ...
  if (0 != result) {
    printf("thread(0x%lx) call pthread_create failed,because %s\n",
           pthread_self(), strerror(result));
  } else {
    printf("thread(0x%lx) call pthread_create ok,new thread id is 0x%lx\n",
           pthread_self(), *tidp);
  }
  return result;
}
...
void print_thread_id() {
  printf("current thread id is 0x%lx\n", pthread_self());
}
void thread_join_int(pthread_t tid, int *rval_pptr) {
  int val = pthread_join(tid, rval_pptr);
  if (0 != val) {
    printf("thread 0x%lx call pthread_join(0x%lx,%p) failed,because %s\n",
           pthread_self(), tid, rval_pptr, strerror(val));
  } else {
    if (NULL == rval_pptr) {
      printf("thread 0x%lx call pthread_join(0x%lx,%p) ok\n", pthread_self(), tid,
             rval_pptr);
    } else {
      printf("thread 0x%lx call pthread_join(0x%lx,%p) ok,thread returns %d\n",

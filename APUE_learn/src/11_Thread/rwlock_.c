...
int My_pthread_rwlock_timedrdlock(pthread_rwlock_t *rwlock,
                                  const struct timespect *tsptr) {
  int result = pthread_rwlock_timedrdlock(rwlock, tsptr);
  pid_t pid = pthread_self();
  if (0 != result) {
    printf("thread(0x%x) call pthread_rwlock_timedrdlock(%p,%p) failed,because "
           "%s\n",
...
int My_pthread_rwlock_timedwrlock(pthread_rwlock_t *rwlock,
                                  const struct timespect *tsptr) {
  int result = pthread_rwlock_timedwrlock(rwlock, tsptr);
  pid_t pid = pthread_self();
  if (0 != result) {
    printf("thread(0x%x) call pthread_rwlock_timedwrlock(%p,%p) failed,because "
           "%s\n",
           pid, rwlock, tsptr, strerror(result));
  } else {
...
extern int My_pthread_create(pthread_t *tidp, const pthread_attr_t *attr,
 ...
 * \param arg : 参数，这里是个整数，如果 arg=0 则不加锁；如果 arg=1 则加读锁；如果arg=2 则加读锁；如果 arg=3 则 tryrdloc
 * ； 如果 arg=4 则
 * trywrlock； 如果 arg=5 则 timedrdlock ； 如果 arg=6 则 timedwrlock \return
 * :返回读到的 shared_int 值（如果加锁失败则返回 -1）
 */

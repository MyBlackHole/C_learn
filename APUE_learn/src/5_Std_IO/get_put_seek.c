...
size_t My_fread(void *ptr, size_t size, size_t nobj, FILE *fp) {
  ...
  if (result < nobj) {
    ...
    if (feof(fp)) {
      printf("fread(%p,%zu,%zu,%p) returns %zu , because EOF\n", ptr, size, nobj,
             fp, result);
    } else if (ferror(fp)) {
      printf("fread(%p,%zu,%zu,%p) returns %zu , because an error:%s\n", ptr, size,
             nobj, fp, result, strerror(errno));
    } else {
      printf("fread(%p,%zu,%zu,%p) can not come here\n", ptr, size, nobj, fp);
}

  } else {
    printf("fread(%p,%zu,%zu,%p) ok\n", ptr, size, nobj, fp);
  }
  printf("fread read %zu objects:", result);
...
size_t My_fwrite(const void *ptr, size_t size, size_t nobj, FILE *fp) {
  ...
  if (result < nobj) {
    printf("fwrite(%p,%zu,%zu,%p) returns %zu,because an error :%s\n", ptr, size,
           nobj, fp, result, strerror(errno));
  } else {
    printf("fwrite(%p,%zu,%zu,%p) ok\n", ptr, size, nobj, fp);
  }
  return result;
}
...
off_t My_ftello(FILE *fp) {
  ...
  if (-1 == result) {
    ...
    printf("ftello(%p) ok,offset is %ld\n", fp, result);
}
  return result;
}
int My_fseeko(FILE *fp, off_t offset, int whence) {
  ...
  if (-1 == result) {
    printf("fseeko(%p,%ld,%d) failed,because %s\n", fp, offset, whence,
           strerror(errno));
  } else {
    printf("fseeko(%p,%ld,%d) ok\n", fp, offset, whence);
}
  return result;
}

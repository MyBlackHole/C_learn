 ...
 *   第四章：文件和目录。
 *
 * 这里是 symlink 函数和 readlink 函数的用法。 symlinkat 函数用法类似 symlink
 * 函数用法。 readlinkat 函数用法类似 readlink 函数用法。
 *
 */
...
void test_symlink_readlink() {
  ...
  assert(prepare_file("test", "abcdefg0123456", 14, S_IRWXU) ==
         0);               // 准备 test 文件
  ...
  print_link_file(
      "test_symlink"); // 由于open 是链接跟随，所以这里打印 test 的内容

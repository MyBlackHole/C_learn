...
void on_new_connection(uv_stream_t *server, int status)
{
	...
	if (uv_accept(server, (uv_stream_t *)client) == 0) {
		...
		uv_write2(write_req, (uv_stream_t *)&worker->pipe, &dummy_buf,
			  1, (uv_stream_t *)client, NULL);
		round_robin_counter =
			(round_robin_counter + 1) % child_worker_count;
	}
...
void setup_workers()
{
	...
	strcpy(worker_path + (strlen(worker_path) - strlen("main")), "worker");

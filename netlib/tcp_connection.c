...
int handle_write(void *data) {
  ...
  ssize_t nwrited =
      write(channel->fd, output_buffer->data + output_buffer->readIndex,
...
int tcp_connection_send_buffer(struct tcp_connection *tcpConnection,
  ...
  int result = tcp_connection_send_data(tcpConnection,
                                        buffer->data + buffer->readIndex, size);
...
void tcp_connection_shutdown(struct tcp_connection *tcpConnection) {
  if (shutdown(tcpConnection->channel->fd, SHUT_WR) < 0) {
    yolanda_msgx("tcp_connection_shutdown failed, socket == %d",

...
const struct event_dispatcher poll_dispatcher = {
    ...
    poll_update, poll_dispatch, poll_clear,
};
...
void *poll_init(struct event_loop *eventLoop) {
  ...
  pollDispatcherData->event_set =
      malloc(sizeof(struct pollfd) * INIT_POLL_SIZE);
  // 用-1表示这个数组位置还没有被占用
  int i;
...
int poll_dispatch(struct event_loop *eventLoop, struct timeval *timeval) {
  ...
  if ((ready_number =
           poll(pollDispatcherData->event_set, INIT_POLL_SIZE, timewait)) < 0) {
  ...
  for (i = 0; i < INIT_POLL_SIZE; i++) {
    ...
    //有事件发生
    if (pollfd.revents > 0) {
      yolanda_msgx("get message channel i==%d, fd==%d, %s", i, socket_fd,

...
struct http_response *http_response_new() {
  ...
  httpResponse->response_headers =
      malloc(sizeof(struct response_header) * INIT_RESPONSE_HEADER_SIZE);
  httpResponse->response_headers_number = 0;
  httpResponse->keep_connected = 0;
  return httpResponse;
}

void http_response_encode_buffer(struct http_response *httpResponse,
  ...
  if (httpResponse->response_headers != NULL &&
      httpResponse->response_headers_number > 0) {

...
struct http_request *http_request_new() {
  ...
  httpRequest->request_headers =
      malloc(sizeof(struct http_request) * INIT_REQUEST_HEADER_SIZE);
...
//给request增加header
void http_request_add_header(struct http_request *httpRequest, char *key,
                             char *value) {
  httpRequest->request_headers[httpRequest->request_headers_number].key = key;
  httpRequest->request_headers[httpRequest->request_headers_number].value =
      value;
...
http_request_current_state(struct http_request *httpRequest) {

...
{
    double area = (rect->max[0]) - (rect->min[0]);
...
static bool nums_equal(NUMTYPE a, NUMTYPE b) { return !(a < b || a > b); }
                                                      ...
                                                      struct node *left)
{
    ...
    {
        double min_dist = left->rects[i].min[axis] - rect->min[axis];
        double max_dist = rect->max[axis] - left->rects[i].max[axis];
...
{
    double area =
        MAX(rect->max[0], other->max[0]) - MIN(rect->min[0], other->min[0]);
    for (int i = 1; i < DIMS; i++)
    {
        area *=
            MAX(rect->max[i], other->max[i]) - MIN(rect->min[i], other->min[i]);
    }
    return area;
}
...
{
    while (index > 0 &&
           node->rects[index].min[0] < node->rects[index - 1].min[0])
    {
...
static void _rtree_search(struct rtree *tr, const NUMTYPE *min,
                          const NUMTYPE *max,
                          bool (*iter)(const NUMTYPE *min, const NUMTYPE *max,
                                       const DATATYPE data, void *udata),
                          void *udata)
...
static size_t _rtree_count(struct rtree *tr) { return tr->count; }

static void node_delete(struct rtree *tr, struct rect *nr, struct node *node,
                        struct rect *ir, struct item item, bool *removed,
                        bool *shrunk,
    ...
    *shrunk = false;
    if (node->kind == LEAF)
    {
        for (int i = 0; i < node->count; i++)
        {
            ...
            int cmp = compare ? compare(node->items[i].data, item.data, udata)
                              : memcmp(&node->items[i].data, &item.data,
                                       sizeof(DATATYPE));
            if (cmp != 0)
            {
                continue;
...
static void node_write_svg(struct node *node, struct rect *rect, FILE *f,
                           int depth)
{
    ...
    if (node)
    {
        if (node->kind == BRANCH)
        {
            for (int i = 0; i < node->count; i++)
            {
                node_write_svg(node->children[i], &node->rects[i], f,
                               depth + 1);
            }
        }
        else
    ...
    {
        double w =
            (rect->max[0] - rect->min[0] + 1 / svg_scale) * svg_scale * 10;
        fprintf(f,
                "<rect x=\"%f\" y=\"%f\" width=\"%f\" height=\"%f\" "
                "fill=\"%s\" fill-opacity=\"1\" "
                "rx=\"3\" ry=\"3\"/>\n",
                (rect->min[0]) * svg_scale - w / 2,
                (rect->min[1]) * svg_scale - w / 2, w, w,
    ...
    else
    {
        fprintf(f,
                ...
                (rect->min[0]) * svg_scale, (rect->min[1]) * svg_scale,
                (rect->max[0] - rect->min[0] + 1 / svg_scale) * svg_scale,
                (rect->max[1] - rect->min[1] + 1 / svg_scale) * svg_scale,
                strokes[depth % nstrokes], strokes[depth % nstrokes], 1);
    }
}
...
static void _rtree_write_svg(struct rtree *tr, const char *path)
{
    ...
    fprintf(f,
            "<svg viewBox=\"%.0f %.0f %.0f %.0f\" "
            "xmlns =\"http://www.w3.org/2000/svg\">\n",
            -190.0 * svg_scale, -100.0 * svg_scale, 380.0 * svg_scale,
            190.0 * svg_scale);
...
struct rtree *rtree_new() { return _rtree_new(); }
...
void rtree_free(struct rtree *tr) { _rtree_free(tr); }
...
size_t rtree_count(struct rtree *tr) { return _rtree_count(tr); }
...
void rtree_check(struct rtree *tr) { _rtree_check(tr); }

...
START_TEST(sbf_callback) {
  ...
  uint64_t counter = 0;
  int res =
      sbf_from_filters(&params, sbf_test_callback, &counter, 0, NULL, &sbf);
  fail_unless(res == 0);
...
START_TEST(test_sbf_flush) {
  ...
  bloom_bitmap maps[2];
  bitmap_from_filename("/tmp/mmap_flush.0.data",
                       get_size("/tmp/mmap_flush.0.data"), 1, SHARED,
                       (bloom_bitmap *)&maps);
  bitmap_from_filename("/tmp/mmap_flush.1.data",
                       get_size("/tmp/mmap_flush.1.data"), 1, SHARED,
                       ((bloom_bitmap *)&maps) + 1);

  bloom_bloomfilter filters[2];
  bf_from_bitmap((bloom_bitmap *)&maps, 1, 0, (bloom_bloomfilter *)&filters);
  bf_from_bitmap(((bloom_bitmap *)&maps) + 1, 1, 0,
                 ((bloom_bloomfilter *)&filters) + 1);
  ...
  bloom_sbf sbf2;
  res =
      sbf_from_filters(&params, sbf_make_callback, &next, 2, filter_map, &sbf2);
  fail_unless(res == 0);
...
START_TEST(test_sbf_close_does_flush) {
  ...
  bloom_bitmap maps[2];
  bitmap_from_filename("/tmp/mmap_close.0.data",
                       get_size("/tmp/mmap_close.0.data"), 1, SHARED,
                       (bloom_bitmap *)&maps);
  bitmap_from_filename("/tmp/mmap_close.1.data",
                       get_size("/tmp/mmap_close.1.data"), 1, SHARED,
                       ((bloom_bitmap *)&maps) + 1);
  ...
  res =
      sbf_from_filters(&params, sbf_make_callback, &next, 2, filter_map, &sbf);
  fail_unless(res == 0);

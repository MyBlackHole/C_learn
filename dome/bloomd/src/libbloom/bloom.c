...
// hash 函数
extern void SpookyHash128(const void *key, size_t len, uint64_t seed1,
                          uint64_t seed2, uint64_t *hash1, uint64_t *hash2);

/**
 * Creates a new bloom filter using a given bitmap and k-value.
 * @arg map A bloom_bitmap pointer.
 * @arg k_num The number of hash functions to use. Ignored if the header value
 * is different.
 * @arg new_filter 1 if new, sets the magic byte and does not check it.
 * @arg filter The filter to setup
 * @return 0 for success. Negative for error.
 */
int bf_from_bitmap(bloom_bitmap *map, uint32_t k_num, int new_filter,
...
int bf_fp_probability_for_capacity_size(bloom_filter_params *params) {
  ...
  double fp_prob =
      pow(M_E, -((double)bits / (double)capacity) * (pow(log(2), 2)));

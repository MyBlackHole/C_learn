 ...
 */
static int prefix_mismatch(art_node *n, unsigned char *key, int key_len,
                           int depth)
{
...
void *art_insert(art_tree *t, unsigned char *key, int key_len, void *value)
{
    int old_val = 0;
    void *old =
        recursive_insert(t->root, &t->root, key, key_len, value, 0, &old_val);
    if (!old_val)
        t->size++;
    return old;
}
 ...
 * @return 0 on success.
 */
static int leaf_prefix_matches(art_leaf *n, unsigned char *prefix,
                               int prefix_len)
{
 ...
 * @return 0 on success, or the return of the callback.
 */
int art_iter_prefix(art_tree *t, unsigned char *key, int key_len,

...
static void copy_item(size_t elsz, struct node *node_a, size_t index_a,
  ...
  memcpy(get_item_at(elsz, node_a, index_a), get_item_at(elsz, node_b, index_b),
         elsz);
}
...
void *btree_load(struct btree *btree, void *item) {
  ...
  if (btree->litem && btree->lnode &&
    ...
    set_item_at(btree->elsize, btree->lnode, (size_t)btree->lnode->num_items,
                item);
  ...
  for (;;) {
    if (node->leaf) {
      btree->lnode = node;
      btree->litem =
          get_item_at(btree->elsize, node, (size_t)(node->num_items - 1));
      break;
    }
...
static bool node_delete(struct btree *btree, struct node *node, enum delact act,
  ...
  if (found) {
    if (act == POPMAX) {
      ...
      copy_item_into(btree->elsize, node, (size_t)i, prev);
      node_delete(btree, node->children[i], POPMAX, 0, NULL, btree->spares[2],
                  hint, depth + 1);
    ...
    deleted = node_delete(btree, node->children[i], act, index, key, prev, hint,
                          depth + 1);
  }
  if (!deleted) {
    return false;
...
void *btree_max(struct btree *btree) {
  ...
  for (;;) {
    if (node->leaf) {
      return get_item_at(btree->elsize, node, (size_t)(node->num_items - 1));
    }
    node = node->children[node->num_items];
  }
}
...
static bool node_ascend(struct btree *btree, struct node *node, void *pivot,
  ...
  if (!found) {
    if (!node->leaf) {
      if (!node_ascend(btree, node->children[i], pivot, iter, udata, hint,
                       depth + 1)) {
        return false;
      }
    }
...
static bool node_descend(struct btree *btree, struct node *node, void *pivot,
  ...
  if (!found) {
    if (!node->leaf) {
      if (!node_descend(btree, node->children[i], pivot, iter, udata, hint,
                        depth + 1)) {
        return false;
      }
    }
...
node_action_ascend(struct btree *btree, struct node *node, void **pivot,
  ...
  for (; i < node->num_items; i++) {
    if (!node->leaf) {
      int ret = node_action_ascend(btree, node->children[i], pivot, iter, udata,
                                   hint, depth + 1);
      if (ret != BTCONTINUE) {
        return ret;
      }
...
node_action_descend(struct btree *btree, struct node *node, void **pivot,
  ...
  if (!node->leaf && !found) {
    int ret = node_action_descend(btree, node->children[i], pivot, iter, udata,
                                  hint, depth + 1);
    if (ret != BTCONTINUE) {
      return ret;
    }
...
void btree_action_ascend_hint(struct btree *btree, void *pivot,
  ...
  while (btree->root) {
    int ret =
        node_action_ascend(btree, btree->root, &pivot, iter, udata, hint, 0);
    if (ret != BTSTARTOVER) {
      break;
    }
  }
...
void btree_action_descend_hint(struct btree *btree, void *pivot,
  ...
  while (btree->root) {
    int ret =
        node_action_descend(btree, btree->root, &pivot, iter, udata, hint, 0);
    if (ret != BTSTARTOVER) {
      break;
    }
  }
...
bool btree_oom(struct btree *btree) { return btree->oom; }
...
enum btree_action pair_delete(void *item, void *udata) { return BTREE_DELETE; }
...
static void test_action_ascend() {
  int max_items =
      getenv("MAX_ITEMS") ? atoi(getenv("MAX_ITEMS")) : def_MAX_ITEMS;
  ...
  struct btree *btree =
      btree_new(sizeof(struct pair), max_items, compare_pairs, nothing);
...
bool simple_iter(const void *item, void *udata) { return true; }
...
static void benchmarks() {
  ...
  bench("load (seq)", N, { btree_load(btree, &vals[i]); }) btree_free(btree);
  ...
  btree = btree_new(sizeof(int), max_items, compare_ints, nothing);
  qsort(vals, N, sizeof(int), compare_ints_nudata);
  bench("set (seq)", N, { btree_set(btree, &vals[i]); }) btree_free(btree);
  ...
  bench("get (seq)", N, { btree_get(btree, &vals[i]); })
      ...
      bench("get (seq-hint)", N, { btree_get_hint(btree, &vals[i], &hint); })
      ...
      bench("max", N, { assert(btree_max(btree)); })
          ...
          bench("ascend", N,
              ...
              bench("descend", N,
                  ...
                  bench("pop-min", N, { btree_pop_min(btree); })

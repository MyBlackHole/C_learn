...
typedef struct {
  ...
  uint32_t dwFilterBits; // m = ceil((n * log(p)) / log(1.0 / (pow(2.0,
                         // log(2.0))))); - BloomFilter的比特数
...
// 计算BloomFilter的参数m,k
static inline void _CalcBloomFilterParam(uint32_t n, double p, uint32_t *pm,
                                         uint32_t *pk) {
  /**
   *  n - Number of items in the filter
   *  p - Probability of false positives, float between 0 and 1 or a number
   *indicating 1-in-p m - Number of bits in the filter k - Number of hash
   *functions
   *
...
inline int InitBloomFilter(BaseBloomFilter *pstBloomfilter, uint32_t dwSeed,
...
// 双重散列封装
FORCE_INLINE void bloom_hash(BaseBloomFilter *pstBloomfilter, const void *key,
                             int len) {
...
FORCE_INLINE int BloomFilter_Add(BaseBloomFilter *pstBloomfilter,
...
FORCE_INLINE int BloomFilter_Check(BaseBloomFilter *pstBloomfilter,
                                   const void *key, int len) {
  if ((pstBloomfilter == NULL) || (key == NULL) || (len <= 0))
    return -1;
...
inline int SaveBloomFilterToFile(BaseBloomFilter *pstBloomfilter,
                                 char *szFileName) {
  ...
  // 接着写入BloomFilter的内容
  iRet =
      fwrite(pstBloomfilter->pstFilter, 1, pstBloomfilter->dwFilterSize, pFile);
...
inline int LoadBloomFilterFromFile(BaseBloomFilter *pstBloomfilter,
                                   char *szFileName) {
  if ((pstBloomfilter == NULL) || (szFileName == NULL))
    return -1;
  ...
  iRet = fread((void *)(pstBloomfilter->pstFilter), 1,
               pstBloomfilter->dwFilterSize, pFile);
  if ((uint32_t)iRet != pstBloomfilter->dwFilterSize) {
    perror("fread(data)");
    return -31;
  }

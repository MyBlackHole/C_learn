 ...
 * IDENTIFICATION
 *    $Header:
 * /usr/local/cvsroot/postgres95/src/backend/postmaster/postmaster.c,v 1.3.2.3
 *1996/10/04 20:33:18 scrappy Exp $
 *
 * NOTES
 *
 * Initialization:
...
int ServerLoop() {
  ...
  for (;;) {
    ...
    if ((nSelected = select(nSockets, &rmask, (fd_set *)NULL, (fd_set *)NULL,
    ...
    if (DebugLvl > 1) {
      ...
      while (curr) {
        ...
        fprintf(stderr, "%s: ServerLoop:\t\tport %d%s pending\n", progname,
                port->sock, FD_ISSET(port->sock, &rmask) ? "" : " not");
        curr = DLGetSucc(curr);
      }
    }
    ...
    while (curr) {
      ...
      if (FD_ISSET(port->sock, &rmask) && port->sock != newFd) {
        ...
        case STATUS_BAD_PACKET:
          ...
          if (DebugLvl)
            fprintf(stderr,
                    "%s: ServerLoop:\t\tbad packet format (reported packet "
                    "size of %d read on port %d\n",
                    progname, port->nBytes, port->sock);
          break;
        case STATUS_NOT_DONE:
          if (DebugLvl)
            fprintf(stderr,
                    "%s: ServerLoop:\t\tpartial packet (%d bytes actually "
                    "read) on %d\n",
                    progname, port->nBytes, port->sock);
          /*
...
static int ConnStartup(Port *port) /* receiving port */
{
  MsgType msgType;
 ...
 */
static void CleanupProc(int pid, int exitstatus) /* child's exit status. */
{
  Dlelem *prev, *curr;
  Backend *bp;
  int sig;
 ...
 *
 */
int BackendStartup(StartupInfo *packet, /* client's startup packet */
                   Port *port, int *pidPtr) {
 ...
 */
void split_opts(char **argv, int *argcp, char *s) {
  int i = *argcp;
...
static int DoExec(StartupInfo *packet, int portFd) {
  ...
  if (DebugLvl > 1) {
    fprintf(stderr, "%s child[%d]: execv(", progname, getpid());
    for (i = 0; i < ac; ++i)
  ...
  fSuccess = CreateProcess(progname, win32_args, NULL, NULL, TRUE, 0, NULL,

 ...
 * IDENTIFICATION
 *    $Header:
 * /usr/local/cvsroot/postgres95/src/backend/access/nbtree/nbtsearch.c,v 1.2
 *1996/07/30 07:56:02 scrappy Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
static OffsetNumber _bt_firsteq(Relation rel, TupleDesc itupdesc, Page page,
                                Size keysz, ScanKey scankey,
                                OffsetNumber offnum);
static int _bt_compare(Relation rel, TupleDesc itupdesc, Page page, int keysz,
                       ScanKey scankey, OffsetNumber offnum);
 ...
 */
BTStack _bt_search(Relation rel, int keysz, ScanKey scankey, Buffer *bufP) {
 ...
 */
Buffer _bt_moveright(Relation rel, Buffer buf, int keysz, ScanKey scankey,
                     int access) {
  Page page;
  BTPageOpaque opaque;
  ItemId hikey;
 ...
 */
bool _bt_skeycmp(Relation rel, Size keysz, ScanKey scankey, Page page,
                 ItemId itemid, StrategyNumber strat) {
  BTItem item;
  ...
  for (i = 1; i <= keysz; i++) {
    ...
    entry = &scankey[i - 1];
    attrDatum = index_getattr(indexTuple, entry->sk_attno, tupDes, &isNull);
 ...
 */
OffsetNumber _bt_binsrch(Relation rel, Buffer buf, int keysz, ScanKey scankey,
                         int srchtype) {
  TupleDesc itupdesc;
  Page page;
  BTPageOpaque opaque;
...
static OffsetNumber _bt_firsteq(Relation rel, TupleDesc itupdesc, Page page,
  ...
  while (offnum > limit && _bt_compare(rel, itupdesc, page, keysz, scankey,
                                       OffsetNumberPrev(offnum)) == 0) {
...
static int _bt_compare(Relation rel, TupleDesc itupdesc, Page page, int keysz,
  ...
  opaque = (BTPageOpaque)PageGetSpecialPointer(page);
  if (!(opaque->btpo_flags & BTP_LEAF) && P_LEFTMOST(opaque) &&
      offnum == P_HIKEY) {
    itemid = PageGetItemId(page, offnum);
    ...
    if (_bt_skeycmp(rel, keysz, scankey, page, itemid, BTEqualStrategyNumber)) {
      return (0);
    }
    return (1);
  }
 ...
 *	pinned.
 */
RetrieveIndexResult _bt_next(IndexScanDesc scan, ScanDirection dir) {
  Relation rel;
  Buffer buf;
  Page page;
  OffsetNumber offnum;
  ...
  if (!BufferIsValid(so->btso_curbuf))
    so->btso_curbuf =
        _bt_getbuf(rel, ItemPointerGetBlockNumber(current), BT_READ);
 ...
 */
RetrieveIndexResult _bt_first(IndexScanDesc scan, ScanDirection dir) {
  Relation rel;
  TupleDesc itupdesc;
  Buffer buf;
  Page page;
  ...
  ScanKeyEntryInitialize(&skdata, 0x0, 1, proc, so->keyData[0].sk_argument);
 ...
 */
bool _bt_step(IndexScanDesc scan, Buffer *bufP, ScanDirection dir) {
  Page page;
  BTPageOpaque opaque;
  OffsetNumber offnum, maxoff;
  OffsetNumber start;

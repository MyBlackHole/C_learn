...
static void _bt_isortcmpinit(Relation index) { _bt_sortrel = index; }

static int _bt_isortcmp(const void *bti1p, const void *bti2p) {
  ...
  if (bti1 == (BTItem)NULL) {
    ...
    return (-1); /* 1 < 2 */
  } else if (_bt_itemcmp(_bt_sortrel, 1, bti1, bti2, BTGreaterStrategyNumber)) {
    return (1); /* 1 > 2 */
  } else if (_bt_itemcmp(_bt_sortrel, 1, bti2, bti1, BTGreaterStrategyNumber)) {
    return (-1); /* 1 < 2 */
  }
  return (0); /* 1 = 2 */
}
...
#define GREATER(a, b) (_bt_isortcmp(&((a)->btpqe_item), &((b)->btpqe_item)) > 0)

static void _bt_pqsift(BTPriQueue *q, int parent) {
  int child;
  BTPriQueueElem e;

  for (child = parent * 2 + 1; child < q->btpq_nelem; child = parent * 2 + 1) {
...
static int _bt_pqnext(BTPriQueue *q, BTPriQueueElem *e) {
...
static void _bt_pqadd(BTPriQueue *q, BTPriQueueElem *e) {
  int child, parent;
...
#define BTITEMSZ(btitem)                                                       \
  ((btitem) ? (IndexTupleDSize((btitem)->bti_itup) +                           \
               (sizeof(BTItemData) - sizeof(IndexTupleData)))                  \
            : 0)
#define SPCLEFT(tape) (sizeof((tape)->bttb_data) - (tape)->bttb_top)
#define EMPTYTAPE(tape) ((tape)->bttb_ntup <= 0)
 ...
 * empty.)
 */
static void _bt_tapereset(BTTapeBlock *tape) {
  tape->bttb_eor = 0;
  tape->bttb_top = 0;
  tape->bttb_ntup = 0;
}
 ...
 */
static void _bt_taperewind(BTTapeBlock *tape) {
 ...
 */
static void _bt_tapeclear(BTTapeBlock *tape) {
 ...
 */
static BTTapeBlock *_bt_tapecreate(char *fname) {
 ...
 */
static void _bt_tapedestroy(BTTapeBlock *tape) {
 ...
 */
static void _bt_tapewrite(BTTapeBlock *tape, int eor) {
  tape->bttb_eor = eor;
 ...
 */
static int _bt_taperead(BTTapeBlock *tape) {
  int fd;
  int nread;
 ...
 */
static BTItem _bt_tapenext(BTTapeBlock *tape, char **pos) {
  Size itemsz;
  BTItem bti;
 ...
 */
static void _bt_tapeadd(BTTapeBlock *tape, BTItem item, int itemsz) {
 ...
 * files.
 */
void *_bt_spoolinit(Relation index, int ntapes) {
  char *mktemp();
 ...
 */
static void _bt_spoolflush(BTSpool *btspool) {
  int i;
 ...
 * output tapes.
 */
static void _bt_spoolswap(BTSpool *btspool) {
  File tmpfd;
  BTTapeBlock *itape;
  BTTapeBlock *otape;
  int i;
...
static void _bt_buildadd(Relation index, BTPageState *state, BTItem bti,
  ...
  if (pgspc < btisz) {
    ...
    for (o = first_off, n = P_FIRSTKEY; o <= last_off;
         o = OffsetNumberNext(o), n = OffsetNumberNext(n)) {
      ii = PageGetItemId(opage, o);
      (void)PageAddItem(npage, PageGetItem(opage, ii), ii->lp_len, n, LP_USED);
#ifdef FASTBUILD_DEBUG
      {
        bool isnull;
        BTItem tmpbti = (BTItem)PageGetItem(npage, PageGetItemId(npage, n));
 ...
 */
static BlockNumber _bt_merge(Relation index, BTSpool *btspool) {
  BTPageState state;

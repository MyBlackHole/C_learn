 ...
 * IDENTIFICATION
 *    $Header:
 * /usr/local/cvsroot/postgres95/src/backend/access/nbtree/nbtutils.c,v 1.2
 *1996/07/30 07:56:04 scrappy Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
ScanKey _bt_mkscankey(Relation rel, IndexTuple itup) {
  ...
  for (i = 0; i < natts; i++) {
    ...
    ScanKeyEntryInitialize(&skey[i], 0x0, (AttrNumber)(i + 1), proc, arg);
  }

  return (skey);
}

void _bt_freeskey(ScanKey skey) { pfree(skey); }
 ...
 */
void _bt_orderkeys(Relation relation, uint16 *numberOfKeys, ScanKey key,
                   uint16 *qual_ok) {
  ScanKey xform;
  ScanKeyData *cur;
  StrategyMap map;
  ...
  for (i = *numberOfKeys; --i >= 0;) {
    ...
    if (init[j]) {
      ...
      test = (long)FMGR_PTR2(cur->sk_func, cur->sk_procedure, cur->sk_argument,
                             xform[j].sk_argument);
      if (test)
  ...
  /* only one of <, <= */
  if (init[BTLessStrategyNumber - 1] && init[BTLessEqualStrategyNumber - 1]) {
...
BTItem _bt_formitem(IndexTuple itup) {
  int nbytes_btitem;
  BTItem btitem;
  Size tuplen;
  extern Oid newoid();

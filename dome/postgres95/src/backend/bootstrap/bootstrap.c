 ...
 * IDENTIFICATION
 *    $Header:
 * /usr/local/cvsroot/postgres95/src/backend/bootstrap/bootstrap.c,v 1.4.2.2
 *1996/08/19 18:04:44 scrappy Exp $
 *
 *-------------------------------------------------------------------------
 */
...
#else
  fprintf(stderr,
          "Usage: postgres -boot [-d] [-C] [-O] [-Q] [-P portno] [dbName]\n");
#endif
...
void boot_openrel(char *relname) {
  ...
  if (Typ == (struct typmap **)NULL) {
    ...
    for (i = 0; PointerIsValid(tup = heap_getnext(sdesc, 0, (Buffer *)NULL));
         ++i)
      ;
    heap_endscan(sdesc);
    ...
    *app = (struct typmap *)NULL;
    ...
    while (PointerIsValid(tup = heap_getnext(sdesc, 0, (Buffer *)NULL))) {
      ...
      memmove((char *)&(*app++)->am_typ, (char *)GETSTRUCT(tup),
              sizeof((*app)->am_typ));
    }
    heap_endscan(sdesc);
    heap_close(rdesc);
  ...
  for (i = 0; i < numattr; i++) {
    ...
    memmove((char *)attrtypes[i], (char *)reldesc->rd_att->attrs[i],
    ...
    if (namestrcmp(&attrtypes[i]->attname, "attisset") == 0)
      attrtypes[i]->attisset =
          get_attisset(reldesc->rd_id, attrtypes[i]->attname.data);
    else
    ...
    if (DebugMode) {
      ...
      printf("create attribute %d name %.*s len %d num %d type %d\n", i,
             NAMEDATALEN, at->attname.data, at->attlen, at->attnum,
             at->atttypid);
      fflush(stdout);
    }
  }
  ...
  if (name) {
    if (reldesc) {
      if (namestrcmp(RelationGetRelationName(reldesc), name) != 0)
        elog(WARN, "closerel: close of '%s' when '%s' was expected", name,
             relname);
    } else
...
void DefineAttr(char *name, char *type, int attnum) {
  ...
  if (Typ != (struct typmap **)NULL) {
    ...
    if (!Quiet)
      printf("<%.*s %s> ", NAMEDATALEN, attrtypes[attnum]->attname.data, type);
 ...
 * ----------------
 */
void InsertOneValue(Oid objectid, char *value, int i) {
  int typeindex;
  char *prt;
  struct typmap **app;
  ...
  if (Typ != (struct typmap **)NULL) {
    ...
    values[i] = fmgr(ap->am_typ.typinput, value, ap->am_typ.typelem,
                     -1); /* shouldn't have char() or varchar() types
                             during boostrapping but just to be safe */
    prt = fmgr(ap->am_typ.typoutput, values[i], ap->am_typ.typelem);
    if (!Quiet)
      printf("%s ", prt);
    pfree(prt);
  } else {
    ...
    values[i] =
        fmgr(Procid[typeindex].inproc, value, Procid[typeindex].elem, -1);
    prt = fmgr(Procid[typeindex].outproc, values[i], Procid[typeindex].elem);
    if (!Quiet)
      printf("%s ", prt);
    pfree(prt);
  }
...
bool BootstrapAlreadySeen(Oid id) {
 ...
 * ----------------
 */
char *LexIDStr(int ident_num) { return (strtable[ident_num]); }
 ...
 * ----------------
 */
int CompHash(char *str, int len) {
  register int result;

  result =
      (NUM * str[0] + NUMSQR * str[len - 1] + NUMCUBE * str[(len - 1) / 2]);
 ...
 * ----------------
 */
hashnode *FindStr(char *str, int length, hashnode *mderef) {
  hashnode *node;
 ...
 * ----------------
 */
hashnode *AddStr(char *str, int strlength, int mderef) {
  ...
  if (++strtable_end == STRTABLESIZE) {
    ...
    elog(FATAL, "There are too many string constants and identifiers for the "
                "compiler to handle.");
  }
 ...
 */
void index_register(char *heap, char *ind, int natts, AttrNumber *attnos,

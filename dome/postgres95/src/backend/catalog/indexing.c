 ...
 * IDENTIFICATION
 *    $Header:
 * /usr/local/cvsroot/postgres95/src/backend/catalog/indexing.c,v 1.1.1.1
 *1996/07/09 06:21:15 scrappy Exp $
 *
 *-------------------------------------------------------------------------
 */
...
char *Name_pg_proc_indices[Num_pg_proc_indices] = {ProcedureNameIndexData.data,
                                                   ...
                                                   ProcedureSrcIndexData.data};char
*Name_pg_type_indices[Num_pg_type_indices] = {TypeNameIndexData.data,
...
char *Name_pg_attr_indices[Num_pg_attr_indices] = {
    AttributeNameIndex, AttributeNumIndex, AttributeRelidIndex};
char *Name_pg_proc_indices[Num_pg_proc_indices] = {
    ProcedureNameIndex, ProcedureOidIndex, ProcedureSrcIndex};
char *Name_pg_type_indices[Num_pg_type_indices] = {TypeNameIndex, TypeOidIndex};
char *Name_pg_class_indices[Num_pg_class_indices] = {ClassNameIndex,
                                                     ClassOidIndex};

static HeapTuple CatalogIndexFetchTuple(Relation heapRelation, Relation idesc,
                                        ScanKey skey);
 ...
 */
void CatalogOpenIndices(int nIndices, char *names[], Relation idescs[]) {
  int i;
 ...
 */
void CatalogCloseIndices(int nIndices, Relation *idescs) {
  int i;
 ...
 * each catalog index.
 */
void CatalogIndexInsert(Relation *idescs, int nIndices, Relation heapRelation,
                        HeapTuple heapTuple) {
  ...
  for (i = 0; i < nIndices; i++) {
    ...
    pgIndexTup = SearchSysCacheTuple(INDEXRELID,
                                     Int32GetDatum(idescs[i]->rd_id), 0, 0, 0);
    Assert(pgIndexTup);
    ...
    FormIndexDatum(natts, (AttrNumber *)&pgIndexP->indkey[0], heapTuple,
                   heapDescriptor, InvalidBuffer, &datum, nulls, finfoP);

    newIndxTup = (IndexTuple)index_formtuple(indexDescriptor, &datum, nulls);
    Assert(newIndxTup);
    /*
 ...
 */
bool CatalogHasIndex(char *catName, Oid catId) {
  Relation pg_class;
  HeapTuple htup;
 ...
 */
HeapTuple AttributeNameIndexScan(Relation heapRelation, Oid relid,
                                 char *attname) {
  Relation idesc;
  ScanKeyData skey;
  OidName keyarg;
  ...
  ScanKeyEntryInitialize(&skey, (bits16)0x0, (AttrNumber)1,
                         (RegProcedure)OidNameEqRegProcedure, (Datum)keyarg);
...
HeapTuple AttributeNumIndexScan(Relation heapRelation, Oid relid,
                                AttrNumber attnum) {
  Relation idesc;
  ScanKeyData skey;
  OidInt2 keyarg;
  ...
  ScanKeyEntryInitialize(&skey, (bits16)0x0, (AttrNumber)1,
                         (RegProcedure)OidInt2EqRegProcedure, (Datum)keyarg);
...
HeapTuple ProcedureOidIndexScan(Relation heapRelation, Oid procId) {
  Relation idesc;
  ScanKeyData skey;
  HeapTuple tuple;
...
HeapTuple ProcedureNameIndexScan(Relation heapRelation, char *procName,
  ...
  ScanKeyEntryInitialize(&skey, (bits16)0x0, (AttrNumber)1,
                         (RegProcedure)NameEqualRegProcedure, (Datum)procName);
  ...
  do {
    ...
    if (indexRes) {
      ...
      break;
  } while (!HeapTupleIsValid(tuple) || pgProcP->pronargs != nargs ||
...
HeapTuple ProcedureSrcIndexScan(Relation heapRelation, text *procSrc) {
  Relation idesc;
  IndexScanDesc sd;
  ScanKeyData skey;
  RetrieveIndexResult indexRes;
  ...
  ScanKeyEntryInitialize(&skey, (bits16)0x0, (AttrNumber)Anum_pg_proc_prosrc,
                         (RegProcedure)TextEqualRegProcedure, (Datum)procSrc);
...
HeapTuple TypeOidIndexScan(Relation heapRelation, Oid typeId) {
  Relation idesc;
  ScanKeyData skey;
  HeapTuple tuple;
...
HeapTuple TypeNameIndexScan(Relation heapRelation, char *typeName) {
  Relation idesc;
  ScanKeyData skey;
  HeapTuple tuple;

  ScanKeyEntryInitialize(&skey, (bits16)0x0, (AttrNumber)1,
                         (RegProcedure)NameEqualRegProcedure, (Datum)typeName);
...
HeapTuple ClassNameIndexScan(Relation heapRelation, char *relName) {
  Relation idesc;
  ScanKeyData skey;
  HeapTuple tuple;
...
HeapTuple ClassOidIndexScan(Relation heapRelation, Oid relId) {
  Relation idesc;
  ScanKeyData skey;
  HeapTuple tuple;

  ScanKeyEntryInitialize(&skey, (bits16)0x0, (AttrNumber)1,
                         (RegProcedure)ObjectIdEqualRegProcedure, (Datum)relId);

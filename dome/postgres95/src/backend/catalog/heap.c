 ...
 * IDENTIFICATION
 *    $Header:
 * /usr/local/cvsroot/postgres95/src/backend/catalog/heap.c,v 1.1.1.1 1996/07/09
 *06:21:15 scrappy Exp $
 *
 * INTERFACE ROUTINES
...
static FormData_pg_attribute a1 = {0xffffffff,
                                   {"ctid"},
                                   27l,
                                   0l,
                                   0l,
...
static FormData_pg_attribute a2 = {
    0xffffffff, {"oid"}, 26l,         0l,
    0l,         0l,      sizeof(Oid), ObjectIdAttributeNumber,
    0,          '\001',  '\001',      0l,
    'i'};
...
static FormData_pg_attribute a4 = {0xffffffff,
                                   {"cmin"},
                                   29l,
                                   0l,
                                   0l,
...
static FormData_pg_attribute a5 = {0xffffffff,
                                   {"xmax"},
                                   28l,
                                   0l,
                                   0l,
...
static FormData_pg_attribute a6 = {0xffffffff,
                                   {"cmax"},
                                   29l,
                                   0l,
                                   0l,
...
static FormData_pg_attribute a8 = {0xffffffff,
                                   {"anchor"},
                                   27l,
                                   0l,
                                   0l,
...
static FormData_pg_attribute a9 = {0xffffffff,
                                   {"tmin"},
                                   20l,
                                   0l,
                                   0l,
...
static FormData_pg_attribute a10 = {0xffffffff,
                                    {"tmax"},
                                    20l,
                                    0l,
                                    0l,
...
static FormData_pg_attribute a11 = {
    0xffffffff, {"vtype"}, 18l,          0l,
    0l,         0l,        sizeof(char), VersionTypeAttributeNumber,
    0,          '\001',    '\001',       0l,
    'c'};

static AttributeTupleForm HeapAtt[] = {&a1, &a2, &a3, &a4,  &a5, &a6,
                                       &a7, &a8, &a9, &a10, &a11};
 ...
 * ----------------------------------------------------------------
 */
Relation heap_creatr(char *name, unsigned smgr, TupleDesc tupDesc) {
  register unsigned i;
  Oid relid;
  Relation rdesc;
  int len;
  ...
  if (IsSystemRelationName(relname) && IsNormalProcessingMode()) {
    elog(WARN, "Illegal class name: %s -- pg_ is reserved for system catalogs",
         relname);
  }
...
static void CheckAttributeNames(TupleDesc tupdesc) {
  ...
  for (i = 0; i < natts; i += 1) {
    for (j = 0; j < sizeof HeapAtt / sizeof HeapAtt[0]; j += 1) {
      if (nameeq(&(HeapAtt[j]->attname), &(tupdesc->attrs[i]->attname))) {
        elog(WARN, "create: system attribute named \"%s\"",
             HeapAtt[j]->attname.data);
      }
    }
    if (tupdesc->attrs[i]->atttypid == UNKNOWNOID) {
      elog(NOTICE, "create: attribute named \"%s\" has an unknown type",
  ...
  for (i = 1; i < natts; i += 1) {
    for (j = 0; j < i; j += 1) {
      if (nameeq(&(tupdesc->attrs[j]->attname),
        ...
        elog(WARN, "create: repeated attribute \"%s\"",
 ...
 * --------------------------------
 */
int RelationAlreadyExists(Relation pg_class_desc, char relname[]) {
  ScanKeyData key;
   ...
   * ----------------
   */
  ScanKeyEntryInitialize(&key, 0, (AttrNumber)Anum_pg_class_relname,
                         (RegProcedure)NameEqualRegProcedure, (Datum)relname);

  /* ----------------
   *	begin the scan
   * ----------------
   */
  pg_class_scan = heap_beginscan(pg_class_desc, 0, NowTimeQual, 1, &key);
  ...
  return (PointerIsValid(tup) == true);
}
...
static void AddNewAttributeTuples(Oid new_rel_oid, TupleDesc tupdesc) {
  ...
  for (i = 0; i < natts; i++) {
    ...
    tup =
        heap_addheader(Natts_pg_attribute, ATTRIBUTE_TUPLE_SIZE, (char *)*dpp);
 ...
 * --------------------------------
 */
void AddPgRelationTuple(Relation pg_class_desc, Relation new_rel_desc,
   ...
   * ----------------
   */
  tup = heap_addheader(Natts_pg_class_fixed, CLASS_TUPLE_SIZE,
                       (char *)new_rel_reltup);
 ...
 * --------------------------------
 */
void addNewRelationType(char *typeName, Oid new_rel_oid) {
  Oid new_type_oid;
 ...
 * --------------------------------
 */
Oid heap_create(char relname[], char *typename, /* not used currently */
                int arch, unsigned smgr, TupleDesc tupdesc) {
   ...
   * ----------------
   */
  AddPgRelationTuple(pg_class_desc, new_rel_desc, new_rel_oid, arch, natts);
...
void RelationRemoveInheritance(Relation relation) {
  ...
  scan = heap_beginscan(catalogRelation, false, NowTimeQual, 1, &entry);
...
void RelationRemoveIndexes(Relation relation) {
  ...
  scan = heap_beginscan(indexRelation, false, NowTimeQual, 1, &entry);
...
void DeletePgRelationTuple(Relation rdesc) {
   ...
   * ----------------
   */
  ScanKeyEntryInitialize(&key, 0, ObjectIdAttributeNumber, F_INT4EQ,
...
void DeletePgAttributeTuples(Relation rdesc) {
   ...
   * ----------------
   */
  ScanKeyEntryInitialize(&key, 0, Anum_pg_attribute_attrelid, F_INT4EQ,
                         rdesc->rd_att->attrs[0]->attrelid);
...
void DeletePgTypeTuple(Relation rdesc) {
  ...
  pg_type_scan = heap_beginscan(pg_type_desc, 0, NowTimeQual, 1, &key);
  ...
  ScanKeyEntryInitialize(&attkey, 0, Anum_pg_attribute_atttypid, F_INT4EQ,
                         typoid);

  pg_attribute_scan =
      heap_beginscan(pg_attribute_desc, 0, NowTimeQual, 1, &attkey);
...
void heap_destroyr(Relation rdesc) {
...
*/
void RemoveFromTempRelList(Relation r) {
  int i;
...
/*
   ...
   MODIFIES the global variable tempRels
*/
void AddToTempRelList(Relation r) {
  if (!tempRels)
    return;

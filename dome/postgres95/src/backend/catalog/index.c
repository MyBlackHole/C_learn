 ...
 * IDENTIFICATION
 *    $Header:
 * /usr/local/cvsroot/postgres95/src/backend/catalog/index.c,v 1.1.1.1.2.1
 *1996/08/19 13:34:44 scrappy Exp $
 *
 *
 * INTERFACE ROUTINES
...
static void InitializeAttributeOids(Relation indexRelation, int numatts,
                                    Oid indexoid);
static void AppendAttributeTuples(Relation indexRelation, int numatts);
static void UpdateIndexRelation(Oid indexoid, Oid heapoid,
                                FuncIndexInfo *funcInfo, int natts,
                                AttrNumber attNums[], Oid classOids[],
                                Node *predicate);
static void DefaultBuild(Relation heapRelation, Relation indexRelation,
                         int numberOfAttributes, AttrNumber attributeNumber[],
                         IndexStrategy indexStrategy, uint16 parameterCount,
                         Datum parameter[], FuncIndexInfoPtr funcInfo,
                         PredInfo *predInfo);
...
static Oid RelationNameGetObjectId(char *relationName, Relation pg_class,
   ...
   * ----------------
   */
  ScanKeyEntryInitialize(&key, 0, Anum_pg_class_relname, NameEqualRegProcedure,
...
static Oid GetHeapRelationOid(char *heapRelationName, char *indexRelationName) {
  ...
  indoid = RelationNameGetObjectId(indexRelationName, pg_class, false);

  if (OidIsValid(indoid))
    elog(WARN, "Cannot create index: '%s' already exists", indexRelationName);

  /* ----------------
   *	get the object id of the heap relation
   * ----------------
   */
  heapoid = RelationNameGetObjectId(heapRelationName, pg_class, true);
...
static TupleDesc BuildFuncTupleDesc(FuncIndexInfo *funcInfo) {
  ...
  tuple =
      SearchSysCacheTuple(PRONAME, PointerGetDatum(funcname),
   ...
   */
  tuple = SearchSysCacheTuple(TYPOID, ObjectIdGetDatum(retType), 0, 0, 0);
  if (!HeapTupleIsValid(tuple))
...
static TupleDesc ConstructTupleDescriptor(Oid heapoid, Relation heapRelation,
  ...
  for (i = 0; i < numatts; i += 1) {
    ...
    atnum = attNums[i];
    if (atnum > natts)
      elog(WARN, "Cannot create index: attribute %d does not exist", atnum);
    ...
    if (!AttrNumberIsForUserDefinedAttr(atnum)) {
      ...
      if (atnum <= FirstLowInvalidHeapAttributeNumber || atnum >= 0)
        elog(WARN,
             "Cannot create index on system attribute: attribute number out of "
             "range (%d)",
             atnum);
      atind = (-atnum) - 1;
...
static void ConstructIndexReldesc(Relation indexRelation, Oid amoid) {
  ...
  indexRelation->rd_am = AccessMethodObjectIdGetAccessMethodTupleForm(amoid);
...
static Oid UpdateRelationRelation(Relation indexRelation) {
  ...
  tuple = heap_addheader(Natts_pg_class_fixed, sizeof(*indexRelation->rd_rel),
...
static void AppendAttributeTuples(Relation indexRelation, int numatts) {
   ...
   * ----------------
   */
  tuple = heap_modifytuple(tuple, InvalidBuffer, pg_attribute, value, nullv,
                           replace);
  ...
  for (i = 1; i < numatts; i += 1) {
     ...
     * ----------------
     */
    memmove(GETSTRUCT(tuple), (char *)indexTupDesc->attrs[i],
            sizeof(AttributeTupleForm));

    value[Anum_pg_attribute_attnum - 1] = Int16GetDatum(i + 1);

    newtuple = heap_modifytuple(tuple, InvalidBuffer, pg_attribute, value,
                                nullv, replace);
...
static void UpdateIndexRelation(Oid indexoid, Oid heapoid,
  ...
  indexForm->indproc =
      (PointerIsValid(funcInfo)) ? FIgetProcOid(funcInfo) : InvalidOid;
   ...
   * ----------------
   */
  tuple = heap_addheader(Natts_pg_index, itupLen, (char *)indexForm);
 ...
 * ----------------------------------------------------------------
 */
void UpdateIndexPredicate(Oid indexoid, Node *oldPred, Node *predicate) {
  Node *newPred;
  char *predString;
  text *predText;
  Relation pg_index;
  ...
  newPred = NULL;
  if (predicate != NULL) {
    newPred = (Node *)make_orclause(
        lcons(make_andclause((List *)predicate),
              lcons(make_andclause((List *)oldPred), NIL)));
  ...
  ScanKeyEntryInitialize(&entry, 0x0, Anum_pg_index_indexrelid,
                         ObjectIdEqualRegProcedure, ObjectIdGetDatum(indexoid));
 ...
 * ----------------------------------------------------------------
 */
void InitIndexStrategy(int numatts, Relation indexRelation,
  ...
  strategy =
      (IndexStrategy)MemoryContextAlloc((MemoryContext)CacheCxt, strsize);

  if (amsupport > 0) {
    strsize = numatts * (amsupport * sizeof(RegProcedure));
    support =
        (RegProcedure *)MemoryContextAlloc((MemoryContext)CacheCxt, strsize);
  } else {
  ...
  IndexSupportInitialize(strategy, support, attrelid, accessMethodObjectId,
...
void index_create(char *heapRelationName, char *indexRelationName,
                  FuncIndexInfo *funcInfo, Oid accessMethodObjectId,
                  int numatts, AttrNumber attNums[], Oid classObjectId[],
                  uint16 parameterCount, Datum *parameter, Node *predicate) {
   ...
   * ----------------
   */
  indexRelation = heap_creatr(indexRelationName, DEFAULT_SMGR, indexTupDesc);
  ...
  if (PointerIsValid(funcInfo)) {
    ...
    proc_tup =
        SearchSysCacheTuple(PRONAME, PointerGetDatum(FIgetname(funcInfo)),
                            Int32GetDatum(FIgetnArgs(funcInfo)),
                            PointerGetDatum(FIgetArglist(funcInfo)), 0);

    if (!HeapTupleIsValid(proc_tup)) {
      func_error("index_create", FIgetname(funcInfo), FIgetnArgs(funcInfo),
   ...
   * ----------------
   */
  UpdateIndexRelation(indexoid, heapoid, funcInfo, numatts, attNums,
                      classObjectId, predicate);
  ...
  if (IsBootstrapProcessingMode()) {
    ...
    index_build(heapRelation, indexRelation, numatts, attNums, parameterCount,
                parameter, funcInfo, predInfo);
  }
}
 ...
 * ----------------------------------------------------------------
 */
void index_destroy(Oid indexId) {
  Relation indexRelation;
  ...
  ScanKeyEntryInitialize(&entry, 0x0, ObjectIdAttributeNumber,
                         ObjectIdEqualRegProcedure, ObjectIdGetDatum(indexId));
  ;
 ...
 * ----------------
 */
void FormIndexDatum(int numberOfAttributes, AttrNumber attributeNumber[],
                    HeapTuple heapTuple, TupleDesc heapDescriptor,
                    Buffer buffer, Datum *datum, char *nullv,
  ...
  for (i = 1; i <= numberOfAttributes; i += 1) {
    ...
    datum[offset] =
        PointerGetDatum(GetIndexValue(heapTuple, heapDescriptor, offset,
                                      attributeNumber, fInfo, &isNull, buffer));
 ...
 * ----------------
 */
void UpdateStats(Oid relid, long reltuples, bool hasindex) {
  Relation whichRel;
  Relation pg_class;
  HeapScanDesc pg_class_scan;
  HeapTuple htup;
  ...
  if (IsBootstrapProcessingMode()) {
    ...
    newtup = heap_modifytuple(htup, buffer, pg_class, values, nulls, replace);
 ...
 * -------------------------
 */
void FillDummyExprContext(ExprContext *econtext, TupleTableSlot *slot,
...
static void DefaultBuild(Relation heapRelation, Relation indexRelation,
  ...
  while (heapTuple = heap_getnext(scan, 0, &buffer),
    ...
    indexTuple = index_formtuple(indexDescriptor, datum, nullv);
...
void index_build(Relation heapRelation, Relation indexRelation,
                 int numberOfAttributes, AttrNumber attributeNumber[],
                 uint16 parameterCount, Datum *parameter,
  ...
  if (RegProcedureIsValid(procedure))
    (void)fmgr(procedure, heapRelation, indexRelation, numberOfAttributes,
               attributeNumber, RelationGetIndexStrategy(indexRelation),
               parameterCount, parameter, funcInfo, predInfo);
  else
    DefaultBuild(heapRelation, indexRelation, numberOfAttributes,
                 attributeNumber, RelationGetIndexStrategy(indexRelation),
                 parameterCount, parameter, funcInfo, predInfo);
}

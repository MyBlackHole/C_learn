...
static lsquic_conn_ctx_t *on_new_conn_cb(void *ea_stream_if_ctx,
					 lsquic_conn_t *conn);

static void on_conn_closed_cb(lsquic_conn_t *conn);

static lsquic_stream_ctx_t *on_new_stream_cb(void *ea_stream_if_ctx,
...
static void on_read_cb(lsquic_stream_t *stream, lsquic_stream_ctx_t *h)
{
	...
	int response_size = snprintf(response, nr + strlen(server_prefix),
				     "%s%s", server_prefix, buf);
...
{
	char *cert_path =
		"/run/media/black/Data/Documents/c/lsquic_learn/demo2/certs/server.cert";
	char *key_path =
		"/run/media/black/Data/Documents/c/lsquic_learn/demo2/certs/server.key";
...
void process_conns(State *state)
{
	...
	if (lsquic_engine_earliest_adv_tick(state->engine, &diff)) {
		...
		ev_timer_init(&state->conn_watcher, process_conns_cb, timeout,
			      0.);

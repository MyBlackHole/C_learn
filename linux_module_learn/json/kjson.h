...
#ifndef KJSON_MEMORY_BUFFER_SIZE
#define KJSON_MEMORY_BUFFER_SIZE 4096
#endif
 ...
 */
#define kj_alloc(obj, data_len) \
	(obj = kzalloc(sizeof(struct kjson_object_t) + data_len, GFP_KERNEL))
 ...
 */
extern struct kjson_object_t *kjson_lookup_object(struct kjson_container *ctn,
						  const char *key);
 ...
 */
extern int kjson_push_object(struct kjson_container *ctn, const char *key,
			     enum kjson_object_type type, void *data,
			     size_t data_len);
 ...
 */
extern int __kjson_push_object(struct kjson_container *ctn,
 ...
 */
#define kjson_push_integer(ctn, key, T)                                     \
	do {                                                                \
 ...
 */
#define kjson_push_string(ctn, key, T)                              \
	do {                                                        \
		kjson_push_object(ctn, key, KOBJECT_TYPE_STRING, T, \
				  strlen(T));                       \
	} while (0)
 ...
 */
#define kjson_push_container(ctn, key, child_ctn)                           \
	do {                                                                \
		kjson_push_object(ctn, key, KOBJECT_TYPE_OBJECT, child_ctn, \
				  0);                                       \
	} while (0)
 ...
 */
#define kjson_push_integer_array(ctn, key, ...)                              \
	do {                                                                 \
		int64_t arr[] = { __VA_ARGS__ };                             \
		kjson_push_object(ctn, key, KOBJECT_TYPE_INTEGER_ARRAY, arr, \
 ...
 */
#define kjson_push_string_array(ctn, key, ...)                              \
	do {                                                                \
		char *arr[] = { __VA_ARGS__ };                              \
		kjson_push_object(ctn, key, KOBJECT_TYPE_STRING_ARRAY, arr, \
 ...
 */
#define kjson_push_container_array(ctn, key, ...)                            \
	do {                                                                 \
		struct kjson_container *arr[] = { __VA_ARGS__ };             \
		kjson_push_object(ctn, key, KOBJECT_TYPE_OBJECT_ARRAY, arr,  \
				  sizeof(arr) /                              \
					  sizeof(struct kjson_container *)); \
	} while (0)

#define kjson_push_null(ctn, key) \
	kjson_push_object(ctn, key, KOBJECT_TYPE_OBJECT_NULL, 0, 0);

#define kjson_push_true(ctn, key)                                            \
	do {                                                                 \
		int true = 1;                                                \
		kjson_push_object(ctn, key, KOBJECT_TYPE_OBJECT_BOOL, &true, \
				  0);                                        \
	} while (0)

#define kjson_push_false(ctn, key)                                            \
	do {                                                                  \
		int false = 0;                                                \
		kjson_push_object(ctn, key, KOBJECT_TYPE_OBJECT_BOOL, &false, \
				  0);                                         \
	} while (0)
 ...
 */
#define kjson_as_integer_array(obj) \
	(int64_t *)((struct kjson_array_struct *)obj->data)->data

/*
 * Converts a kjson_object_t to an array of string.
 */
#define kjson_as_string_array(obj) \
	(char **)((struct kjson_array_struct *)obj->data)->data

/*
 * Converts a kjson_object_t to an array of containers.
 * kjson_containers returned are childs (Do not deallocated it).
 */
#define kjson_as_container_array(obj)                                       \
	(struct kjson_container **)((struct kjson_array_struct *)obj->data) \
		->data

/*
 * Get the lenght of an array.
 */
#define kjson_array_length(obj) \
	(size_t)((struct kjson_array_struct *)obj->data)->len
...
extern int kjson_dump_to_file(struct kjson_container *ctn, struct file *fp);

extern struct kjson_container *kjson_file_parse(struct file *fp);

...
static struct kjson_object_t *__kjson_create_string(void *data,
						    size_t data_len);
static struct kjson_object_t *__kjson_create_ctn_object(void *data,
							size_t data_len);
static struct kjson_object_t *__kjson_create_integer_array(void *data,
							   size_t data_len);
static struct kjson_object_t *__kjson_create_string_array(void *data,
							  size_t data_len);
static struct kjson_object_t *__kjson_create_ctn_object_array(void *data,
							      size_t data_len);
...
static int kjson_dump_process_container(struct kjson_container *ctn,
					struct kjstring_t *json_dmp);
static int kjson_dump_object(struct kjson_object_t *obj,
			     struct kjstring_t *json_dmp);

static void __kjson_dump_integer(struct kjson_object_t *obj,
				 struct kjstring_t *json_dmp);
static void __kjson_dump_string(struct kjson_object_t *obj,
				struct kjstring_t *json_dmp);
static void __kjson_dump_ctn_object(struct kjson_object_t *obj,
				    struct kjstring_t *json_dmp);
static void __kjson_dump_integer_array(struct kjson_object_t *obj,
				       struct kjstring_t *json_dmp);
static void __kjson_dump_string_array(struct kjson_object_t *obj,
				      struct kjstring_t *json_dmp);
static void __kjson_dump_ctn_object_array(struct kjson_object_t *obj,
					  struct kjstring_t *json_dmp);
static void __kjson_dump_null(struct kjson_object_t *obj,
			      struct kjstring_t *json_dmp);
static void __kjson_dump_bool(struct kjson_object_t *obj,
			      struct kjstring_t *json_dmp);

typedef void (*kjson_dump_ops_t)(struct kjson_object_t *, struct kjstring_t *);

static void copy_string_array(struct kjson_object_t *obj, char **str_array,
			      size_t array_len);
...
struct kjson_object_t *kjson_lookup_object(struct kjson_container *ctn,
					   const char *key)
{
	struct kjson_object_t *obj;
	u32 index;
...
void copy_string_array(struct kjson_object_t *obj, char **str_array,
		       size_t array_len)
{
	char **parr;
	size_t str_size;
...
struct kjson_object_t *__kjson_create_integer_array(void *data, size_t data_len)
{
	...
	if (kj_alloc(obj, (data_len * sizeof(int64_t)) +
				  sizeof(struct kjson_array_struct)) == NULL)
		return NULL;
...
struct kjson_object_t *__kjson_create_string_array(void *data, size_t data_len)
{
	...
	if (kj_alloc(obj, (data_len * sizeof(char *)) +
				  sizeof(struct kjson_array_struct)) == NULL)
		return NULL;
...
struct kjson_object_t *__kjson_create_ctn_object_array(void *data,
						       size_t data_len)
{
	struct kjson_object_t *obj;
	struct kjson_array_struct *s;
	...
	if (kj_alloc(obj, (data_len * sizeof(struct kjson_container *)) +
				  sizeof(struct kjson_array_struct)) == NULL)
		return NULL;
...
static kjson_create_ops_t kj_create_ops[] = {
	__kj_create_integer,	     __kjson_create_string,
	__kjson_create_ctn_object,   __kjson_create_integer_array,
	__kjson_create_string_array, __kjson_create_ctn_object_array,
	__kjson_create_null,	     __kjson_create_bool
};

// void kjson_push_object(struct kjson_container *json_ctn, kjson_type type, const char *key, void *data, ...)
int kjson_push_object(struct kjson_container *ctn, const char *key,
		      enum kjson_object_type type, void *data, size_t data_len)
{
	struct kjson_object_t *obj;
...
#define close_scoope(json_dmp)                  \
	do {                                    \
		kjstring_append(json_dmp, "}"); \
		kjstring_file_write(json_dmp);  \
	} while (0)
...
#define set_string(json_dmp, str)                \
	do {                                     \
		kjstring_append(json_dmp, "\""); \
...
#define set_key(json_dmp, str)                   \
	do {                                     \
		set_string(json_dmp, str);       \
		kjstring_append(json_dmp, ": "); \
	} while (0)

#define set_array(json_dmp, array, array_len, set_FOO)           \
	do {                                                     \
		int i;                                           \
		open_array(json_dmp);                            \
		for (i = 0; i < array_len; i++) {                \
...
#define set_integer_array(json_dmp, array_ptr, array_len) \
	set_array(json_dmp, array_ptr, array_len, set_integer)
#define set_string_array(json_dmp, array_ptrs, array_len) \
	set_array(json_dmp, array_ptrs, array_len, set_string)
#define set_ctn_array(json_dmp, array_ptrs, array_len) \
	set_array(json_dmp, array_ptrs, array_len, set_ctn)

// Do not change the sequence of this array
kjson_dump_ops_t kjson_dump_ops[] = {
	__kjson_dump_integer,	   __kjson_dump_string,
	__kjson_dump_ctn_object,   __kjson_dump_integer_array,
	__kjson_dump_string_array, __kjson_dump_ctn_object_array,
	__kjson_dump_null,	   __kjson_dump_bool
};

void __kjson_dump_integer(struct kjson_object_t *obj,
			  struct kjstring_t *json_dmp)
{
	// set key
...
void __kjson_dump_string(struct kjson_object_t *obj,
			 struct kjstring_t *json_dmp)
{
	// set key
	set_key(json_dmp, obj->key);
...
void __kjson_dump_ctn_object(struct kjson_object_t *obj,
			     struct kjstring_t *json_dmp)
{
...
void __kjson_dump_integer_array(struct kjson_object_t *obj,
				struct kjstring_t *json_dmp)
{
...
void __kjson_dump_string_array(struct kjson_object_t *obj,
			       struct kjstring_t *json_dmp)
{
...
void __kjson_dump_ctn_object_array(struct kjson_object_t *obj,
				   struct kjstring_t *json_dmp)
{
...
int kjson_dump_process_container(struct kjson_container *ctn,
				 struct kjstring_t *json_dmp)
{

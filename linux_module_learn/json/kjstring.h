...
#define kjstring_static_declare(name, size)                      \
	...
	name->buffer_size = size;                                \
	name->off = 0;                                           \
	name->is_file = false;                                   \
	name->str_data = name->__data;                           \
 ...
 */
static inline struct kjstring_t *
kjstring_new_string_buffer(struct kjstring_t *str, char *buffer,
			   size_t buffer_size)
{
	if (!str || !buffer)
		return NULL;
...
static inline struct kjstring_t *kjstring_alloc(size_t default_size)
{
	...
	if (!default_size ||
	    (str = kjstring_allocator(sizeof(struct kjstring_t) +
				      default_size)) == NULL)
		return NULL;
 ...
 */
#define kjstring_clear(str)                                   \
	do {                                                  \
		str->off = 0;                                 \
		memset(str->str_data, 0x0, str->buffer_size); \
	} while (0)

#define kjstring_append_pos(str, src, pos)                       \
	do {                                                     \
		size_t len = strlen(src);                        \
		if (kjstring_end(str, len)) {                    \
			kjstring_file_write(str);                \
		}                                                \
		if (pos < str->buffer_size) {                    \
			size_t len = strlen(src);                \
			strncpy(&str->str_data[pos], src,        \
				(str->buffer_size - pos));       \
...
static inline int kjstring_file_write(struct kjstring_t *str)
{
	loff_t offset = 0;
	ssize_t nwrite = 0;
	char *str_data = str->str_data;
	...
	while (str->off > 0) {
		offset = str->fp->f_pos;
		nwrite = kernel_write(str->fp, str_data, str->off, &offset);
		if (nwrite < 0) {
			break;
		}
		if (nwrite == 0) {
	...
	printk(KERN_INFO " %ld bytes write\n", str_data - str->str_data);
	/*set_fs(old_fs);*/
	if (str->off > 0) {
		return -1;
	}

	return str_data - str->str_data;
}

static inline bool kjstring_end(struct kjstring_t *str, size_t len)
{
	return str->buffer_size > str->off + len;
}
 ...
 */
static inline void kjstring_append_string(struct kjstring_t *str, char *src,
					  size_t pos)
{
 ...
 * @pos: The position to start the append
 */
static inline void kjstring_append_integer(struct kjstring_t *str, int64_t val,
					   size_t pos)
{
...
static inline void __kjstring_no_append(struct kjstring_t *str, int64_t val,
					size_t pos)
{
}

#define kjstring_insert_type(str, src, pos)           \
	_Generic((src),                               \
		char *: kjstring_append_string,       \
		const char *: kjstring_append_string, \
		int64_t: kjstring_append_integer,     \
		default: __kjstring_no_append)(str, src, pos)
 ...
 */
static inline void kjstring_interator_init(const struct kjstring_t *str,
 ...
 */
#define kjstring_copy_iterator(iterator_src, iterator_dest)         \
	do {                                                        \
		(iterator_dest)->str = (iterator_src)->str;         \
		(iterator_dest)->str_tmp = (iterator_src)->str_tmp; \
 ...
 * @chr: An empty char.
 */
#define kjstring_iterator_next(iterator, chr)                                 \
	do {                                                                  \
		chr = '\0';                                                   \
		if (kjstring_iterator_end(iterator)) {                        \
			kjstring_iterator_file_read(iterator);                \
		}                                                             \
...
static inline int kjstring_file_read(struct kjstring_t *str)
{
	loff_t offset = 0;
	ssize_t nread = 0;
	/*mm_segment_t old_fs;*/
	...
	nread = kernel_read(str->fp, str->str_data, str->buffer_size, &offset);
	str->str_data[nread] = '\0';
	printk(KERN_INFO " %ld bytes read, data is %s\n", nread, str->str_data);
	/*set_fs(old_fs);*/
	if (nread > 0) {
		str->off += nread;
		str->fp->f_pos += nread;
...
static inline void
kjstring_iterator_file_read(struct kjstring_iterator *iterator)
{
	ssize_t nread = 0;
	nread = kjstring_file_read(iterator->str_tmp);
	if (nread > 0) {
		iterator->pos = 0;
	}
...
kjstring_iterator_follow(const struct kjstring_iterator *iterator)
{

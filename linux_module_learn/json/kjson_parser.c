...
#define pop_nospace(iterator, chr)                             \
	do {                                                   \
		do {                                           \
...
#define get_nospace(iterator, chr)                             \
	do {                                                   \
...
static struct kjson_container *
kjson_start_parser(struct kjstring_iterator *iterator, bool is_nested);
static struct kjson_object_t *
kjson_parse_object(struct kjstring_iterator *iterator);

/* Parse the key from the iterator of the json string
 * Returns 0 in case of success
 */
static int parse_key(struct kjstring_iterator *iterator,
		     struct kjstring_t *ret_key);
static int parse_integer(struct kjstring_iterator *iterator,
			 int64_t *ret_value);
static int parse_string(struct kjstring_iterator *iterator,
			struct kjstring_t *ret_str);

static struct kjson_object_t *parse_value(struct kjstring_iterator *iterator);

static struct kjson_object_t *
parse_integer_object(struct kjstring_iterator *iterator);
static struct kjson_object_t *
parse_string_object(struct kjstring_iterator *iterator);
static struct kjson_object_t *
parse_ctn_object(struct kjstring_iterator *iterator);

static struct kjson_object_t *
parse_array_object(struct kjstring_iterator *iterator);
static struct kjson_object_t *
parse_string_array(struct kjstring_iterator *iterator);
static struct kjson_object_t *
parse_integer_array(struct kjstring_iterator *iterator);
static struct kjson_object_t *
parse_ctn_array(struct kjstring_iterator *iterator);
static struct kjson_object_t *
parse_null_bool_object(struct kjstring_iterator *iterator);
...
static inline void set_error(char *msg, int64_t pos, char character)
{
	...
	kjstring_append(((struct kjstring_t *)&kjson_parser_error),
			". Position: ");
...
int parse_string(struct kjstring_iterator *iterator, struct kjstring_t *ret_str)
{
	...
	if (nextchar != '\"') {
		set_error("Character not recognized - do you miss '\"' ?",
			  iterator->pos, nextchar);
		return 1;
	}
...
struct kjson_object_t *parse_array_object(struct kjstring_iterator *iterator)
{
	...
	pop_nospace(iterator, nextchar);
	if (nextchar != '[') {
		set_error("Invalid operand on array of objects", iterator->pos,
			  nextchar);
		return NULL;
	}
	...
	case ']': {
		...
		if (kj_alloc(obj, sizeof(struct kjson_array_struct)) == NULL) {
			set_error("Unable to alloc memory", iterator->pos,
				  '\0');
			return NULL;
		}
...
struct kjson_object_t *parse_string_array(struct kjstring_iterator *iterator)
{
	...
	if (kj_alloc(obj, (array_size * sizeof(char *)) +
	...
	for (i = 0; i < array_size; i++) {
		str_size = find_string_size(iterator) +
			   1; // Last 1 byte for Null terminator

		// alloc the buffer for the string to insert into the json container
		if ((parsed_str_buffer[i] = kzalloc(str_size, GFP_KERNEL)) ==
		    NULL) {
			set_error("Unable to alloc memory", iterator->pos,
				  '\0');
			goto FAIL;
		...
		kjstring_new_string_buffer(&parsed_str, parsed_str_buffer[i],
					   str_size);
...
struct kjson_object_t *parse_integer_array(struct kjstring_iterator *iterator)
{
	...
	if (kj_alloc(obj, (array_size * sizeof(int64_t)) +
				  sizeof(struct kjson_array_struct)) == NULL)
		return NULL;
	...
	for (i = 0; i < array_size; i++) {
		if (parse_integer(iterator, (int64_t *)(data_hdr->data +
							(i * sizeof(int64_t)))))
			goto FAIL;
...
struct kjson_object_t *parse_ctn_array(struct kjstring_iterator *iterator)
{
	...
	if (kj_alloc(obj, (array_size * sizeof(struct kjson_container *)) +
				  sizeof(struct kjson_array_struct)) == NULL)
		return NULL;
	...
	for (i = 0; i < array_size; i++) {
		if ((parsed_obj[i] = kjson_start_parser(iterator, true)) ==
		    NULL)
			goto FAIL;
...
parse_null_bool_object(struct kjstring_iterator *iterator)
{
	...
	if (!strncmp(next_str, "null", 4) || !strncmp(next_str, "NULL", 4)) {
		...
		iterator->pos += 4;
	} else if (!strncmp(next_str, "true", 4) ||
		   !strncmp(next_str, "TRUE", 4)) {
		...
		*(int *)obj->data = 1;
		...
		iterator->pos += 4;
	} else if (!strncmp(next_str, "false", 5) ||
		   !strncmp(next_str, "FALSE", 5)) {
		if (kj_alloc(obj, sizeof(int)) == NULL)
			return NULL;
...
struct kjson_container *kjson_start_parser(struct kjstring_iterator *iterator,
					   bool is_nested)
{
	...
	if (IS_ERR(ctn = kjson_new_container())) {
		set_error("Unable to create a new kjson container",
			  iterator->pos, nextchar);
		return NULL;
	}
	...
	if (nextchar_t == '}') {
		...
		if (!is_nested && nextchar != '\0') {
			set_error("Json not recognized - syntax error",
				  iterator->pos, nextchar);
			return NULL;
		} else
			return ctn;
	...
	while (nextchar != '}' && nextchar != '\0') {
		...
		if (__kjson_push_object(ctn, obj)) {
			set_error(
				"Unable to push a new object to the kjson container",
				iterator->pos, nextchar);
	...
	if (nextchar == '\0') {
		set_error("Json ends without anything to parse", iterator->pos,
			  nextchar);
		goto FAIL;
	}
	...
	if (!is_nested) {
		...
		if (nextchar != '\0') {
			set_error(
				"Json contains strange characters after the end scoope",
				iterator->pos, nextchar);
			goto FAIL;
		}
	}
...
struct kjson_container *kjson_file_parse(struct file *fp)
{
...
EXPORT_SYMBOL_GPL(kjson_file_parse);

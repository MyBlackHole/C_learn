 ...
 *
 * call_rcu() 		:  callback function is called when finish to wait every
 * grace periods (async) synchronize_rcu() :  wait to finish every grace periods
 * (sync)
 */
...
static void print_book(int id)
{
    ...
    list_for_each_entry_rcu(b, &books, node)
    {
        if (b->id == id)
        {
             ...
             */
            pr_info(
                "id : %d, name : %s, author : %s, borrow : %d, addr : %lx\n",

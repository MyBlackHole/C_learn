 ...
 * function) into this struct. This makes is easier for setting up the hook and
 * just passing the entire struct off to fh_install_hook() later on.
 * */
struct ftrace_hook {
	const char *name;
	void *function;
				    ...
				    struct ftrace_regs *fregs)
{
	...
	if (!within_module(parent_ip, THIS_MODULE))
		ftrace_regs_set_instruction_pointer(
			fregs, (unsigned long)hook->function);

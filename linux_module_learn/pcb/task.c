...
static int __init traverse_pcb(void)
{
    ...
    list_for_each(pos, &task->tasks)  // 使用list_for_each宏来遍历进程链表
    {
        ...
        */
        printk(
            "comm:%s; pid:%d; prio:%d; static_prio:%d; "
            "parent'pid:%d; count:%d; umask:%d;",
            p->comm, p->pid, p->prio, p->static_prio, (p->parent)->pid,
        ...
        if (files != NULL)
        {
            ...
            while (fdt != NULL)
            {
                for (i = 0; i < NR_OPEN_DEFAULT; i++)
                {
                    ...
                    {
                        printk("NAME:%s", file->f_path.dentry->d_name.name);
                        if (file->f_path.mnt != NULL)
                        {
                            buf_path = d_path(&(file->f_path), buf_path, 512);
                            if (buf_path != NULL)
                            {
                                printk("%s", buf_path);
                            }

...
{
	// 这个结构体如下：
	// struct tree_descr { const char *name; const struct file_operations *ops;
	// int mode; };
...
/*
这个函数是按照内核代码中的样子改的，是struct dentry
*类型，这里是一个封装，这里可以返回好几种函数： 　－　mount_single
　－　mount_bdev　
　－　mount_nodev
*/
...
// 对应于读取的aufs文件的读取方法
static ssize_t myfs_file_read(struct file *file, char __user *buf, size_t count,
			      loff_t *ppos)
{
	int actual_readed;
	int ret;
...
// 对应于写入的aufs文件的写入方法
static ssize_t myfs_file_write(struct file *file, const char __user *buf,
...
static struct file_system_type my_fs_type = { .owner = THIS_MODULE,
					      .name = MYFS,
					      .mount = myfs_get_sb,
...
struct dentry *myfs_creat_dir(const char *name, struct dentry *parent)
{
	...
	return myfs_creat_file(name, S_IFDIR | S_IRWXU | S_IRUGO, parent, NULL,
			       NULL);
}
...
static int __init myfs_init(void)
{
	...
	//@S_IFREG：表示一个文件
	//@S_IRUGO：用户读｜用户组读｜其他读
	myfs_creat_file("one", S_IFREG | S_IRUGO | S_IWUSR, pslot, NULL,
			&myfs_file_operations);
	myfs_creat_file("two", S_IFREG | S_IRUGO | S_IWUSR, pslot, NULL,
			&myfs_file_operations);

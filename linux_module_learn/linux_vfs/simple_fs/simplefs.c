...
static ssize_t simple_fs_read(struct file *filp, char __user *buf, size_t len,
			      loff_t *ppos)
{
...
static ssize_t simple_fs_write(struct file *filp, const char __user *buf,
...
static int simple_fs_readdir_iter(struct file *filp, struct dir_context *ctx)
{
	...
	for (i = 0; i < blk->dir_children; i++) {
		if (!dir_emit(ctx, simple_entry[i].filename,
			      strnlen(simple_entry[i].filename, FILE_NAME),
...
static int simple_fs_do_create(struct user_namespace *mnt_userns,
...
static int simple_fs_mkdir(struct user_namespace *mnt_userns,
			   struct inode *inode, struct dentry *dentry,
			   umode_t mode)
{
	return simple_fs_do_create(mnt_userns, inode, dentry, S_IFDIR | mode);
}

static int simple_fs_create(struct user_namespace *mnt_userns,
			    struct inode *inode, struct dentry *dentry,
			    umode_t mode, bool excl)
{
	return simple_fs_do_create(mnt_userns, inode, dentry, mode);
}
...
static struct dentry *simple_fs_lookup(struct inode *dir, struct dentry *dentry,
				       unsigned int flags)
{
	int i;
	...
	for (i = 0; i < blk->dir_children; i++) {
		if (strcmp(simple_entry[i].filename, dentry->d_name.name) ==
		    0) {
			struct inode *inode =
				simple_fs_iget(sb, simple_entry[i].idx);
			struct block_info *tmp_inode =
				(struct block_info *)inode->i_private;
...
static int simple_fs_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	struct simple_dir_entry *simple_entry;
	struct block_info *blk =
		(struct block_info *)dentry->d_inode->i_private;
	...
	for (i = 0; i < parent_blk->dir_children; i++) {
		if (strcmp(simple_entry[i].filename, dentry->d_name.name) ==
			...
			for (j = i; j < (parent_blk->dir_children - 1); j++) {
				memcpy(&simple_entry[j], &simple_entry[j + 1],
				       sizeof(struct simple_dir_entry));
			}
			parent_blk->dir_children--;
			break;
		}
...
struct file_system_type simple_fs_type = { .owner = THIS_MODULE,
					   .name = "simple_fs",

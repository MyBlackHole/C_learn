...
MODULE_PARM_DESC(virt_boundary,
...
MODULE_PARM_DESC(init_hctx,
                 "Fault injection to fail hctx init. "
                 "init_hctx=<interval>,<probability>,<space>,<times>");
#endif
...
MODULE_PARM_DESC(shared_tag_bitmap,
...
{
    return null_param_store_val(str, &g_irqmode, NULL_IRQ_NONE, NULL_IRQ_TIMER);
}
...
MODULE_PARM_DESC(memory_backed,
                 "Create a memory-backed block device. Default: false");

static bool g_discard;
module_param_named(discard, g_discard, bool, 0444);
MODULE_PARM_DESC(discard,
                 "Support discard operations (requires memory-backed null_blk "
                 "device). Default: false");

static unsigned long g_cache_size;
module_param_named(cache_size, g_cache_size, ulong, 0444);
MODULE_PARM_DESC(
    mbps, "Cache size in MiB for memory-backed device. Default: 0 (none)");

static unsigned int g_mbps;
module_param_named(mbps, g_mbps, uint, 0444);
MODULE_PARM_DESC(mbps,
                 "Limit maximum bandwidth (in MiB/s). Default: 0 (no limit)");

static bool g_zoned;
module_param_named(zoned, g_zoned, bool, S_IRUGO);
MODULE_PARM_DESC(
    zoned, "Make device as a host-managed zoned block device. Default: false");

static unsigned long g_zone_size = 256;
module_param_named(zone_size, g_zone_size, ulong, S_IRUGO);
MODULE_PARM_DESC(zone_size,
                 "Zone size in MB when block device is zoned. Must be "
                 "power-of-two: Default: 256");

static unsigned long g_zone_capacity;
module_param_named(zone_capacity, g_zone_capacity, ulong, 0444);
MODULE_PARM_DESC(zone_capacity,
                 "Zone capacity in MB when block device is zoned. Can be less "
                 "than or equal to zone size. Default: Zone size");
...
MODULE_PARM_DESC(zone_max_open,
                 "Maximum number of open zones when block device is zoned. "
                 "Default: 0 (no limit)");

static unsigned int g_zone_max_active;
module_param_named(zone_max_active, g_zone_max_active, uint, 0444);
MODULE_PARM_DESC(zone_max_active,
                 "Maximum number of active zones when block device is zoned. "
                 "Default: 0 (no limit)");
...
static ssize_t nullb_device_uint_attr_store(unsigned int *val, const char *page,
                                            size_t count)
{
    unsigned int tmp;
    int result;
...
#define NULLB_DEVICE_ATTR(NAME, TYPE, APPLY)                                   \
    static ssize_t nullb_device_##NAME##_show(struct config_item *item,        \
                                              char *page)                      \
    {                                                                          \
        return nullb_device_##TYPE##_attr_show(to_nullb_device(item)->NAME,    \
                                               page);                          \
    }                                                                          \
    static ssize_t nullb_device_##NAME##_store(struct config_item *item,       \
                                               const char *page, size_t count) \
    {                                                                          \
                                            ...
                                            const char *page, size_t count)
{
    ...
    *tmp = '\0';
    ...
    if (buf[0] == '+')
        ret = badblocks_set(&t_dev->badblocks, start, end - start + 1, 1);
    else
        ret = badblocks_clear(&t_dev->badblocks, start, end - start + 1);
    if (ret == 0)
        ret = count;
out:
    kfree(orig);
...
static struct config_item *nullb_group_make_item(struct config_group *group,
                                                 const char *name)
{
...
static void nullb_group_drop_item(struct config_group *group,
...
static void null_free_device_storage(struct nullb_device *dev, bool is_cache)
{
    ...
    do
    {
        ...
        nr_pages =
            radix_tree_gang_lookup(root, (void **)t_pages, pos, FREE_BATCH);
...
static struct nullb_page *null_lookup_page(struct nullb *nullb, sector_t sector,
                                           bool for_write, bool ignore_cache)
{
...
static struct nullb_page *null_insert_page(struct nullb *nullb, sector_t sector,
                                           bool ignore_cache)
    __releases(&nullb->lock) __acquires(&nullb->lock)
{
    u64 idx;
    struct nullb_page *t_page;
...
static int null_flush_cache_page(struct nullb *nullb, struct nullb_page *c_page)
{
    ...
    if (test_bit(NULLB_PAGE_FREE, c_page->bitmap))
    {
        ...
        {
            ret = radix_tree_delete_item(&nullb->dev->data, idx, t_page);
            null_free_page(t_page);
        }
        return 0;
    }
    ...
    for (i = 0; i < PAGE_SECTORS; i += (nullb->dev->blocksize >> SECTOR_SHIFT))
    {
        if (test_bit(i, c_page->bitmap))
        {
            offset = (i << SECTOR_SHIFT);
            memcpy(dst + offset, src + offset, nullb->dev->blocksize);
...
again:
    if ((nullb->dev->cache_size * 1024 * 1024) > nullb->dev->curr_cache + n ||
        nullb->dev->curr_cache == 0)
        return 0;
                           ...
                           unsigned int off, sector_t sector, size_t n)
{
    ...
    while (count < n)
    {
        ...
        offset = (sector & SECTOR_MASK) << SECTOR_SHIFT;
        t_page =
            null_lookup_page(nullb, sector, false, !null_cache_active(nullb));
...
static int null_handle_flush(struct nullb *nullb)
{
    ...
    {
        err =
            null_make_cache_space(nullb, nullb->dev->cache_size * 1024 * 1024);
                         ...
                         sector_t sector, bool is_fua)
{
    ...
    {
        if (dev->zoned)
            valid_len = null_zone_valid_read_len(nullb, sector, len);

        if (valid_len)
        {
            err = copy_from_nullb(nullb, page, off, sector, valid_len);
            off += valid_len;
            len -= valid_len;
        }
...
static void nullb_zero_read_cmd_buffer(struct nullb_cmd *cmd)
{
    ...
    {
        __rq_for_each_bio(bio, cmd->rq) zero_fill_bio(bio);
    }
}
...
static struct nullb_queue *nullb_to_queue(struct nullb *nullb)
{
    ...
    if (nullb->nr_queues != 1)
        index = raw_smp_processor_id() /
                ((nr_cpu_ids + nullb->nr_queues - 1) / nullb->nr_queues);
...
static void null_map_queues(struct blk_mq_tag_set *set)
{
    ...
    if (nullb)
    {
         ...
         */
        if (set->nr_hw_queues == dev->submit_queues + dev->poll_queues)
        {
                                  ...
                                  const struct blk_mq_queue_data *bd)
{
    ...
    cmd->nq = nq;
    cmd->fake_timeout =
        should_timeout_request(rq) || blk_should_fake_timeout(rq->q);
...
static void cleanup_queues(struct nullb *nullb)
{
    ...
    for (i = 0; i < nullb->nr_queues; i++) cleanup_queue(&nullb->queues[i]);
...
static int setup_queues(struct nullb *nullb)
{
    ...
    nullb->queues = kcalloc(nqueues, sizeof(struct nullb_queue), GFP_KERNEL);
    if (!nullb->queues)
        return -ENOMEM;
...
static int null_validate_conf(struct nullb_device *dev)
{
    ...
    if (dev->memory_backed)
        ...
        dev->cache_size = 0;
    dev->cache_size =
        min_t(unsigned long, ULONG_MAX / 1024 / 1024, dev->cache_size);
    dev->mbps = min_t(unsigned int, 1024 * 40, dev->mbps);
    /* can not stop a queue */
    if (dev->queue_mode == NULL_Q_BIO)
        dev->mbps = 0;

    if (dev->zoned && (!dev->zone_size || !is_power_of_2(dev->zone_size)))
    {
...
static int null_add_dev(struct nullb_device *dev)
{
    ...
    if (config_item_name(&dev->item))
    {
        ...
        snprintf(nullb->disk_name, sizeof(nullb->disk_name), "%s",
                 config_item_name(&dev->item));
    }
    else
    {

...
#define NULLB_DEVICE_ATTR(NAME, TYPE, APPLY)                                 \
	...
	static ssize_t nullb_device_##NAME##_store(                          \
		struct config_item *item, const char *page, size_t count)    \
	{                                                                    \
		int (*apply_fn)(struct nullb_device * dev, TYPE new_value) = \
			APPLY;                                               \
		struct nullb_device *dev = to_nullb_device(item);            \
		TYPE new_value = 0;                                          \
		int ret;                                                     \
                                                                             \
		ret = nullb_device_##TYPE##_attr_store(&new_value, page,     \
						       count);               \
		if (ret < 0)                                                 \
			return ret;                                          \
		if (apply_fn)                                                \
					    ...
					    const char *page, size_t count)
{
	...
	*tmp = '\0';
	...
	if (buf[0] == '+')
		ret = badblocks_set(&t_dev->badblocks, start, end - start + 1,
				    1);
	else
...
static void null_free_device_storage(struct nullb_device *dev, bool is_cache)
{
	...
	do {
		...
		nr_pages = radix_tree_gang_lookup(root, (void **)t_pages, pos,
						  FREE_BATCH);
...
static int null_flush_cache_page(struct nullb *nullb, struct nullb_page *c_page)
{
	...
	if (test_bit(NULLB_PAGE_FREE, c_page->bitmap)) {
		...
		if (t_page && null_page_empty(t_page)) {
			ret = radix_tree_delete_item(&nullb->dev->data, idx,
						     t_page);
			null_free_page(t_page);
		}
		return 0;
...
static int null_handle_flush(struct nullb *nullb)
{
	...
	spin_lock_irq(&nullb->lock);
	while (true) {
		err = null_make_cache_space(nullb, nullb->dev->cache_size *
							   1024 * 1024);
			 ...
			 sector_t sector, bool is_fua)
{
	...
	if (!is_write) {
		if (dev->zoned)
			valid_len =
				null_zone_valid_read_len(nullb, sector, len);

		if (valid_len) {
			err = copy_from_nullb(nullb, page, off, sector,
					      valid_len);
			off += valid_len;
			len -= valid_len;
		}
...
static int setup_queues(struct nullb *nullb)
{
	...
	nullb->queues =
		kcalloc(nqueues, sizeof(struct nullb_queue), GFP_KERNEL);
	if (!nullb->queues)
		return -ENOMEM;

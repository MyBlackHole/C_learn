...
#define LOG(fmt, ...)                                                \
    ...
    {                                                                \
        if (ctx->fp)                                                 \
            fprintf(ctx->fp, "%s: " fmt, __FUNCTION__, __VA_ARGS__); \
    } while (0)
...
int sbtopen(void) { return (-1); }

int sbtread(void) { return (-1); }

int sbtwrite(void) { return (-1); }

int sbtclose(void) { return (-1); }

int sbtinfo(void) { return (-1); }

int sbtremove(void) { return (-1); }
...
int catenate(char *buf, size_t n, const char *dir, const char *file)
{
    ...
    return snprintf(buf, n, "%s/%s", dir, path);
}
             ...
             struct sbtinfo **output)
{
    ...
    while (*file_ref_ref)
    {
        ...
        // Media sharing mode
        *(value_ref = (int *)malloc(4)) = 1;
        ctx->pairs[index].type          = SBTINFO_SHARE;
        ctx->pairs[index].value         = value_ref;
        ++index;
            ...
            struct sbtopt **output)
{
    ...
    global.options[index].value = "Test SBT";
    ++index;

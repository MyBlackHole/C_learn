...
void virtio_disk_init()
{
	...
	*R(VIRTIO_MMIO_QUEUE_READY) = 1;
	...
	disk.avail =
		(struct virtq_avail *)(disk.pages + NUM * sizeof(virtq_desc_t));
...
void virtio_disk_rw(struct blk *b, int write)
{
	...
	buf0->reserved =
		0; // The reserved portion is used to pad the header to 16 bytes and move the 32-bit sector field to the correct place.
	...
	*R(VIRTIO_MMIO_QUEUE_NOTIFY) =
		0; // The device starts immediately when we write 0 to queue_notify.
	while (b->disk == 1) {
	}

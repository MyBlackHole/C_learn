 ...
 *this example on OpenGL ES 2.0 platforms (Android, Raspberry Pi, HTML5), use
 *#version 100 shaders raylib comes with shaders ready for both versions, check
 *raylib/shaders install folder
 *
...
int main(void) {
  ...
  Texture2D texture = LoadTexture(
      "resources/models/barracks_diffuse.png"); // Load model texture (diffuse
                                                // map)
  model.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture =
      texture; // Set model diffuse texture
  ...
  Shader shader = LoadShader(
      0, TextFormat("resources/shaders/glsl%i/swirl.fs", GLSL_VERSION));

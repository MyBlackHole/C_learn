...
typedef struct MemLeakT
{  // 将所有malloc和calloc申请的内存串起来
    MemInfo mem_info;
...
UNUSED static void report_to_file()
{
    ...
    if (fp_write != NULL)
    {
        ...
        for (MemLeak *leak_info = g_mem_leak_start; leak_info != NULL;
...
static void report_to_log()
{
    ...
    else
    {
             ...
             leak_info = leak_info->next)
        {
            ...
            log_info("place   : %s:%d", leak_info->mem_info.file_name,
                     leak_info->mem_info.line);
...
static void regest_mem_leak_detection_report()
{
    atexit(mem_leak_detection_report);
}
...
void *mem_leak_detection_malloc(size_t size, const char *file,
                                unsigned int line)
{
    pthread_once(&g_mem_leak_detection_key_once,
                 regest_mem_leak_detection_report);

...
StudentDao *student_dao_create(void)
{
	...
	list_push_back(priv->students,
		       list_node_new((int64_t)student_create("Robert", 0)));
	list_push_back(priv->students,
		       list_node_new((int64_t)student_create("John", 1)));
...
{
	if (NULL == student_dao || NULL == *student_dao)
		return;
	StudentDao *self = *student_dao;
	priv_t *priv = (priv_t *)self->priv;
	if (NULL != priv && NULL != priv->students)
		list_destroy(priv->students);
	freep(&self->priv);
...
{
	if (NULL == student_dao || NULL == student_dao->priv)
		return NULL;
				...
				Student *student)
{
	priv_t *priv = student_dao->priv;
	ListNode *node =
		list_at(priv->students, student_get_roll_number(student));

...
static void print_persons(List *persons)
{
	...
	while ((node = list_iterator_next(it))) {
		...
		printf("Person : [ Name : %s, Gender : %s, Marital Status : %s ]\n",
		       person_get_name(person), person_get_gender(person),
		       person_get_marital_status(person));
	}
...
int main(int argc, char const *argv[])
{
	...
	list_push_back(persons, list_node_new((int64_t)person_create(
					"Robert", "Male", "Single")));
	list_push_back(persons, list_node_new((int64_t)person_create(
					"John", "Male", "Married")));
	list_push_back(persons, list_node_new((int64_t)person_create(
	...
	List *single_or_female_list =
		criteria_meet_criteria(single_or_female, persons);

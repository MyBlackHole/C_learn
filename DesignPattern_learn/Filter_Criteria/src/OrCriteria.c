...
static void init(struct Criteria *criteria, struct Criteria *other_criteria1,
		 struct Criteria *other_criteria2)
{
	if (NULL == criteria->priv)
		freep((void **)&criteria->priv);
...
{
	priv_t *priv = (priv_t *)criteria->priv;
	List *first_criteria_items =
		criteria_meet_criteria(priv->other_criteria1, persons);
	List *other_criteria_items =
		criteria_meet_criteria(priv->other_criteria2, persons);

	ListIterator *it = list_iterator_new(other_criteria_items, LIST_HEAD);
	ListNode *node;
	while ((node = list_iterator_next(it))) {
		ListNode *node_to_find =
			list_find(first_criteria_items, node->val);
		if (NULL == node_to_find) {
			list_push_back(first_criteria_items,
				       list_node_new(node->val));
		}
	}
...
struct Criteria *criteria_or_create(struct Criteria *other_criteria1,
				    struct Criteria *other_criteria2)
{

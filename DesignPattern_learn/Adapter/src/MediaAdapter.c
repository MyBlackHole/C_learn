...
static void destroy_advanced_media_player(
    struct AdvancedMediaPlayer **advanced_media_player) {
  if (NULL == advanced_media_player || NULL == *advanced_media_player) {
    return;
}
...
static void init(struct MediaPlayer *media_player) {
  media_player->priv = (priv_t *)calloc(1, sizeof(priv_t));
}
...
static void play(struct MediaPlayer *media_player, const char *audio_type,
  ...
  if (0 == strcasecmp("vlc", audio_type)) {
    ...
    if (NULL != priv->advanced_music_player->play_vlc) {
      priv->advanced_music_player->play_vlc(priv->advanced_music_player,
                                            file_name);
    }
    ...
    if (NULL != priv->advanced_music_player->play_mp4) {
      priv->advanced_music_player->play_mp4(priv->advanced_music_player,
                                            file_name);
    }
  }
}

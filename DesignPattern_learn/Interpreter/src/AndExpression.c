...
static void init_with_expressions(struct Expression *expression,
				  struct Expression *expression1,
				  struct Expression *expression2)
{
...
{
	if (NULL == expression || NULL == expression->priv)
		return;
...
static bool interpret(struct Expression *expression, const char *context)
{
	...
	if (NULL == priv->expression1 || NULL == priv->expression2)
		return false;
	if (NULL == priv->expression1->interpret ||
	    NULL == priv->expression2->interpret) {
		return false;
	}
...
struct Expression *and_expression_create(struct Expression *expression1,
					 struct Expression *expression2)
{

...
static void shape_set_id(struct Shape *shape, const int64_t id)
{
	shape->id = id;
}

static int64_t shape_get_id(struct Shape *shape)
{
	return shape->id;
}

static const char *shape_get_type(struct Shape *shape)
{
	return shape->type;
}
...
{
	struct Shape *new_shape = rectangle_create();
	return new_shape;
}

...
static struct Iterator *name_iterator_create(void)
{
	struct Iterator *iterator =
		(struct Iterator *)calloc(1, sizeof(struct Iterator));
	if (NULL == iterator)
		return NULL;
...
NameRepository *name_repository_create(void)
{
	NameRepository *name_repository =
		(NameRepository *)calloc(1, sizeof(NameRepository));
	if (NULL == name_repository)
		return NULL;
...
{
	if (NULL == name_repository || NULL == *name_repository)
		return;

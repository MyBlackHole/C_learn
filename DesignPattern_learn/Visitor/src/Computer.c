...
{
	if (NULL == computer_part || NULL == computer_part->priv) {
		return;
	}
		   ...
		   struct ComputerPartVisitor *computer_part_visitor)
{
	...
	for (int i = 0; i < 3; ++i) {
		if (NULL != priv->parts[i] && NULL != priv->parts[i]->accept) {
			priv->parts[i]->accept(priv->parts[i],
					       computer_part_visitor);
		}
	}
	computer_part_visitor_visit_computer(computer_part_visitor,
					     computer_part);
}

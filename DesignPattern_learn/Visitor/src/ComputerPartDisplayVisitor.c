...
static void visit_computer(struct ComputerPartVisitor *computer_part_visitor,
			   struct ComputerPart *computer)
{
	printf("Displaying Computer.\n");
}

static void visit_mouse(struct ComputerPartVisitor *computer_part_visitor,
			struct ComputerPart *mouse)
{
	printf("Displaying Mouse.\n");
}

static void visit_keyboard(struct ComputerPartVisitor *computer_part_visitor,
			   struct ComputerPart *keyboard)
{
	printf("Displaying Keyboard.\n");
}

static void visit_monitor(struct ComputerPartVisitor *computer_part_visitor,
...
struct ComputerPartVisitor *computer_part_display_visitor_create(void)
{
	struct ComputerPartVisitor *computer_part_display_visitor =
		(struct ComputerPartVisitor *)calloc(
			1, sizeof(struct ComputerPartVisitor));
	if (NULL == computer_part_display_visitor) {
		return NULL;
	}

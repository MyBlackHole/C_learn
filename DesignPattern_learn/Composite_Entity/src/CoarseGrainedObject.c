...
CoarseGrainedObject* coarse_grained_object_create(void)
{
    CoarseGrainedObject* coarse_grained_object =
        (CoarseGrainedObject*)calloc(1, sizeof(CoarseGrainedObject));
    if (NULL == coarse_grained_object)
        return NULL;
...
{
    if (NULL == coarse_grained_object || NULL == *coarse_grained_object)
        return;
...
void coarse_grained_object_set_data(CoarseGrainedObject* coarse_grained_object,
...
const char** coarse_grained_object_get_data(
    CoarseGrainedObject* coarse_grained_object, int* data_length)
{

...
CompositeEntity *composite_entity_create(void)
{
	CompositeEntity *composite_entity =
		(CompositeEntity *)calloc(1, sizeof(CompositeEntity));
	if (NULL == composite_entity)
		return NULL;

	composite_entity->coarse_grained_object =
		coarse_grained_object_create();
	return composite_entity;
}

void composite_entity_destroy(CompositeEntity **composite_entity)
{
	if (NULL == composite_entity || NULL == *composite_entity)
		return;
			       ...
			       const char *data1, const char *data2)
{
	if (NULL == composite_entity)
		return;
	coarse_grained_object_set_data(composite_entity->coarse_grained_object,
				       data1, data2);
}
				       ...
				       int *data_length)
{
	...
	return coarse_grained_object_get_data(
		composite_entity->coarse_grained_object, data_length);
}

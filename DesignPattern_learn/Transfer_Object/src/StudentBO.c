...
static bool match_func(int64_t a, int64_t b)
{
	...
	return (student_vo_get_roll_number(student_a) ==
		student_vo_get_roll_number(student_b));
}
...
StudentBO *student_bo_create(void)
{
	...
	student_bo->students->free_func =
		(void (*)(int64_t))student_vo_free_func;
	list_push_back(student_bo->students,
		       list_node_new((int64_t)student_vo_create("Robert", 0)));
	list_push_back(student_bo->students,
		       list_node_new((int64_t)student_vo_create("John", 1)));
	return student_bo;
}

void student_bo_destroy(StudentBO **student_bo)
{
	if (NULL == student_bo || NULL == *student_bo)
		return;
...
{
	if (NULL == student_bo || NULL == student_vo)
		return;
	ListNode *node_to_delete =
		list_find(student_bo->students, (int64_t)student_vo);

...
BusinessDelegate* business_delegate_create(void)
{
    BusinessDelegate* business_delegate =
        (BusinessDelegate*)calloc(1, sizeof(BusinessDelegate));
    if (NULL == business_delegate)
        return NULL;
...
{
    if (NULL == business_delegate || NULL == *business_delegate)
        return;
...
void business_delegate_do_task(BusinessDelegate* business_delegate)
{
    if (NULL == business_delegate)
        return;
    BusinessService* business_service = business_look_up_get_business_service(
        business_delegate->lookup_service, business_delegate->service_type);

...
CareTaker *care_taker_create(void)
{
	...
	care_taker->memento_list->free_func =
		(void (*)(int64_t))memento_destroy;
	return care_taker;
}

void care_taker_destroy(CareTaker **care_taker)
{
	if (NULL == care_taker || NULL == *care_taker)
		return;
...
{
	if (NULL == care_taker || NULL == care_taker->memento_list)
		return;

	list_push_back(care_taker->memento_list,
		       list_node_new((int64_t)memento));
}

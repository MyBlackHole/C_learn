...
StudentController *student_controller_create(Student *model, StudentView *view)
{
	StudentController *controller =
		(StudentController *)calloc(1, sizeof(StudentController));
	if (NULL == controller)
		return NULL;
...
{
	if (NULL == controller || NULL == *controller)
		return;
	freep((void **)controller);
}

void student_controller_set_student_name(StudentController *controller,
					 const char *name)
{
	if (NULL == controller)
		return;
...
void student_controller_set_student_roll_number(StudentController *controller,
...
const char *
student_controller_get_student_roll_number(StudentController *controller)
{
	if (NULL == controller)
		return NULL;
...
void student_controller_update_view(StudentController *controller)
{
	if (NULL == controller)
		return;
	student_view_print_student_details(
		controller->view, student_get_name(controller->model),

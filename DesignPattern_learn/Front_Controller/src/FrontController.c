...
FrontController *front_controller_create(void)
{
	FrontController *front_controller =
		(FrontController *)calloc(1, sizeof(FrontController));
	if (NULL == front_controller)
		return NULL;
...
{
	if (NULL == front_controller || NULL == *front_controller)
		return;
...
static void track_request(const char *request)
{

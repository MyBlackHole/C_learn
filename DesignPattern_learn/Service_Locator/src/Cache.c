...
{
	if (NULL == cache || NULL == cache->services)
		return NULL;
...
void cache_add_service(Cache *cache, Service *new_service)
{
	...
	while ((node = list_iterator_next(it))) {
		Service *service = (Service *)node->val;
		if (0 == strcasecmp(service_get_name(new_service),
	...
	if (!exists)
		list_push_back(cache->services,
			       list_node_new((int64_t)new_service));
}

...
//         （1）编号为 0 至 [n/2-1] 的结点为分支结点， 编号为 [n/2] 至 n-1
//         的结点为叶子结点； （2）当 n 为奇数则每
//         分支结点既有左孩子又有右孩子，当 n 为偶数则每个分支结点只有左孩子没有右孩子 （3）对于每个编号为
//         
//         i
//         的分支结点，其左孩子结点的编号
//
//         i+1，右孩子结点的编号为 2i+2
//         （4）除编号为0的堆顶结点外，对于其余编号为 i
//         的结点，其双亲结点的编号为 [(i-1)/2]
...
void InsertHeap(struct HeapSq *HBT, ElemType x) {
  ...
  HBT->len++;              //堆长度加1
  i = HBT->len -
      1; // i指向待调整元素的位置，即其数组下标，初始指向新元素所在的堆尾位置
  while (i != 0) {
    int j = (i - 1) / 2; // j指向下标为i的元素的双亲
    if (x >=
        HBT->heap[j]) //若新元素大于待调整元素的双亲，则比较调整结束，退出循环
      break;
...
ElemType DeleteHeap(struct HeapSq *HBT) {
  ...
  while (
      j <=
      HBT->len -
          1) //寻找待调整元素的最终位置，每次使孩子元素上移一层，调整到孩子为空时止
  {
    if (j < HBT->len - 1 &&
        HBT->heap[j] > HBT->heap[j + 1]) //若存在右孩子且较小，使j指向右孩子
      j++;

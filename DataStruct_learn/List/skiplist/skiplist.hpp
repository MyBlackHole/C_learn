...
class skiplist{
        ...
        skiplist() {
            ...
            head = new
            node_type(head_key, MAX_LEVEL);
            nil = new
            node_type(nil_key, MAX_LEVEL);
        ...
        ~skiplist() {
            node_type *node = head;
            while (nullptr != node and
            nullptr != node->forwards[0]) {
                node_type *tmp = node;
        ...
        node_type* get_first_equal(const value_type& v) const {
            ...
            for (size_type i = get_node_level(head); i > 0; --i) {
                while (x->forwards[i - 1]->key < target orx->forwards[i - 1]->key == target
                and
                x->forwards[i - 1]->value != v) {
                    x = x->forwards[i - 1];
                }
            }
            return x->forwards[0];
...
typename Value
>
const typename skiplist<Value>::value_type
skiplist<Value>::default_value =
typename

skiplist<Value>::value_type();

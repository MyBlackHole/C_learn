...
#define skiplist_foreach_forward(pos, end) for (; pos != end; pos = (pos)->next)
...
static void skipnode_delete(struct skipnode *node) { free(node); }
...
static struct skipnode *first_in_range(struct skiplist *list,
 ...
 * where min and max are inclusive. */
static struct skipnode *last_in_range(struct skiplist *list,
                                      struct range_spec *range) {
  int i = list->level - 1;
  struct sk_link *pos = &list->head[i];
  struct sk_link *end = &list->head[i];
...
static struct skipnode *skiplist_search_by_rank(struct skiplist *list,
                                                int rank) {

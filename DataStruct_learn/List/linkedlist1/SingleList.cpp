...
private:
  void Insert(CElement *lpNewElement, CElement *lpCurElement,
              bool bBack = true);
...
CElement *CSingleList::Insert(CElement *lpElement, void *lpData,
                              int iDataSize) {
...
CElement *CSingleList::End() { return m_lpNull; }

bool CSingleList::Empty() { return Begin() == End(); }
 ...
 * 合并的2个链表必须是有序的
 */
void CSingleList::Merge(CSingleList &lst,
                        std::function<int(void *t1, void *t2)> fnCompare) {
  CElement *lpL1 = Begin();
...
void *CElement::GetDataPtr() { return m_lpData; }

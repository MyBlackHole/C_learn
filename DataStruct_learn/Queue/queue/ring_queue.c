...
// 通过求余实现环回
int _valid_index(int curr, int step, int cap) { return (curr + step) % cap; }

// 下一个元素位置
int _next(int curr, int cap) { return _valid_index(curr, 1, cap); }

// 是否有可取元素
bool is_empty(struct ring_queue *queue) { return (queue->head == queue->tail); }

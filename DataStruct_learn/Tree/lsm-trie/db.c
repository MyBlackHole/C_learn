...
static bool db_dump_meta(struct DB *const db) {
  ...
  for (int i = 0; db->cms_dump[i]; i++) {
    ...
    sprintf(path_cm_dump, "./%s/%s-%01d-%018.6lf", DB_META_BACKUP_DIR,
            DB_META_CMAP_PREFIX, i, sec0);
...
static uint64_t db_table_dump(struct DB *const db, struct Table *const table,
  ...
  assert(rdm);
  db_log_diff(db, sec0, "DUMP @%lu [%8lx #%08lx] [%08lu] %s", start_bit / 3,
              mtid, off_main / TABLE_ALIGN, nr_items, buffer);
  return mtid;
}
...
static void compaction_feed_all(struct Compaction *const comp) {
  for (uint64_t i = 0; i < comp->nr_feed; i++) {
    ...
    db_log_diff(comp->db, sec0, "FEED @%lu [%8lx #%08lx]", comp->start_bit / 3,
                comp->mts_old[i]->mtid,
...
static void compaction_update_vc(struct Compaction *const comp) {
  ...
  for (uint64_t i = 0; i < 8; i++) {
    const bool ri =
        vc_insert_internal(vc->sub_vc[i], comp->mts_new[i], comp->mbcs_new[i]);
    assert(ri);
  }
...
static void *thread_compaction(void *ptr) {
  ...
  while (true) {
    ...
    // get token
    const uint64_t token =
        (__sync_fetch_and_add(&(db->compaction_token), 1)) % DB_COMPACTION_NR;
...
static struct ContainerMapConf *db_load_cm_conf(const char *const fn) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    // hint * GB
    cm_conf->hints[i] = hint * UINT64_C(1024) * UINT64_C(1024) * UINT64_C(1024);
    count++;
  }

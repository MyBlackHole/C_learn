...
{
	// 确保是 BLOOMTABLE_INTERVAL 倍数
	const uint64_t nr_offsets =
		(nr_bf + BLOOMTABLE_INTERVAL - 1u) / BLOOMTABLE_INTERVAL;
	// 创建 BloomTable 内存结构
	struct BloomTable *const bt = (typeof(bt))malloc(
		sizeof(*bt) + (nr_offsets * sizeof(bt->offsets[0])));
	// 断言
	assert(bt);
	...
	for (uint64_t i = 0; i < nr_bf; i++) {
		...
		// p - buf 得到 bf->bytes 占用多少个 uint8_t
	...
	for (uint64_t i = 0; i < nr_bf; i++) {
		...
		struct BloomFilter *const bf = bfs[i];
		// 填充 bf->bytes 到 ptr 里，返回下一个 uint8_t 起始指针
...
bool bloomtable_dump(struct BloomTable *const bt, FILE *const fo)
{
	...
	const size_t nw =
		fwrite(bt->raw_bf, sizeof(bt->raw_bf[0]), nr_btbytes, fo);
					    ...
					    struct Stat *const stat)
{
	...
	for (uint64_t i = 0; i < bt->nr_bf; i++) {
		...
		// write box
		uint16_t *const pboxid_new =
			(typeof(pboxid_new))(page + off_page);
		*pboxid_new = (uint16_t)i;
		uint16_t *const pboxlen_new =
			(typeof(pboxlen_new))(page + off_page +
					      sizeof(*pboxid_new));
		*pboxlen_new = boxlen_new;
		uint8_t *const pbox_new = page + off_page +
					  sizeof(*pboxid_new) +
					  sizeof(*pboxlen_new);
	...
	memcpy(bc->index_last, index_last,
					     ...
					     struct Stat *const stat)
{
	...
	for (uint64_t i = 0; i < bt->nr_bf; i++) {
		...
		if (i > bc->index_last[old_page]) {
			...
			const ssize_t nbi =
				pread(bc->raw_fd, old, BARREL_ALIGN,
				      bc->off_raw + (old_page * BARREL_ALIGN));
		...
		if (off_page + alllen_new >
		    BARREL_ALIGN) { // switch to next page
			if (off_page < BARREL_ALIGN) {
				bzero(page + off_page, BARREL_ALIGN - off_page);
			}
			page += BARREL_ALIGN;
	...
	const ssize_t nr_raw_bytes =
		(typeof(nr_raw_bytes))(current_page * BARREL_ALIGN);
	const ssize_t nrb =
		pwrite(new_raw_fd, pages, nr_raw_bytes, new_off_raw);
	assert(nrb == nr_raw_bytes);
	huge_free(pages, pages_cap);
	stat_inc_n(&(stat->nr_write_bc), current_page);
			      ...
			      const uint64_t barrel_id, uint8_t *const buf)
{
	for (uint64_t i = 0; i < bc->nr_index; i++) {
		if (bc->index_last[i] >= barrel_id) {
			// fetch page at [i]
			const ssize_t nr =
				pread(bc->raw_fd, buf, BARREL_ALIGN,
...
bool bloomcontainer_dump_meta(struct BloomContainer *const bc, FILE *const fo)
{
	...
	assert(nbpb == 1);
	const size_t nnri =
		fwrite(&(bc->nr_index), sizeof(bc->nr_index), 1, fo);
	assert(nnri == 1);
	const size_t nidx = fwrite(bc->index_last, sizeof(bc->index_last[0]),
				   bc->nr_index, fo);
						...
						const int raw_fd)
{
	...
	assert(noff == 1);
	const size_t nbar =
		fread(&(bc0.nr_barrels), sizeof(bc0.nr_barrels), 1, fi);
	assert(nbar == 1);
	const size_t nbpb =
	...
	const size_t nidx = fread(bc->index_last, sizeof(bc->index_last[0]),
				  bc->nr_index, fi);
	assert(nidx == bc->nr_index);
	return bc;
}
			      ...
			      const uint32_t index, const uint64_t hv)
{
	...
	for (;;) {
		...
		if (id == index) {
			// match one by one
			uint8_t *pbox = (typeof(pbox))(ptr + sizeof(*pid) +
						       sizeof(*plen));

...
void uncached_probe_test(void)
{
	...
	printf("probe %lu times over 1GB, %luusec, %.2lf p/s\n", times, dt,
	       ((double)times) * 1000000.0 / ((double)dt));
}
...
void false_positive_test(void)
{
	...
	for (int i = 0; i < 32; i++) {
		...
		uint64_t *const keys =
			(typeof(keys))malloc(sizeof(keys[0]) * nr_keys);
...
void multi_level_false_positive_test(void)
{
	...
	uint64_t *const keys =
		(typeof(keys))malloc(sizeof(keys[0]) * nr_all_keys);
	...
	for (uint64_t l = 0; l < level; l++) {
		for (uint64_t k = 0; k < nr_keys; k++) {
			const bool m =
				bloom_match(bfs[l], keys[level * nr_keys + k]);
			if (m) {
				fp++;
			}
		}
...
void containertest(void)
{
	...
	// bc
	const int rawfd = open("/tmp/bctest",
			       O_CREAT | O_TRUNC | O_RDWR | O_LARGEFILE, 00666);
	assert(rawfd >= 0);
	...
	for (uint64_t i = 0; i < xcap; i++) {
		for (uint64_t j = 0; j < 64; j++) {
			...
			assert(m);
			const uint8_t n =
				bloomcontainer_match(bcs[0], i, sha + 1);
			if (n) {
				match++;
			} else {
				nomatch++;
	...
	for (uint64_t i = 0; i < xcap; i++) {
		for (uint64_t j = 0; j < 64; j++) {
			...
			const uint8_t m0 =
				bloomcontainer_match(bcs[1], i, sha0);
			if (m0 < 4) {
				match01[m0]++;
			}
			const uint64_t h1 = i + (j << 20) + (j << 30) +
					    (j << 40) + (UINT64_C(1) << 50);
			SHA1((const unsigned char *)(&h1), 8, hash);
			const uint64_t sha1 = *((uint64_t *)(&hash[7]));
			const uint8_t m1 =
				bloomcontainer_match(bcs[1], i, sha1);
			if (m1 < 4) {
				match01[m1]++;
			}
		}
	...
	for (uint64_t z = 0; z < 8; z++) {
		for (uint64_t i = 0; i < xcap; i++) {
			for (uint64_t j = 0; j < 64; j++) {
				...
				const uint64_t sha = *((uint64_t *)(&hash[7]));
				const uint8_t m =
					bloomcontainer_match(bcs[7], i, sha);
				assert(m);
				mc[z]++;
				if ((m & (1 << z)) == 0) {
					mismatch++;
				}
				const uint8_t n = bloomcontainer_match(
					bcs[7], i, sha + 1);
				if (n) {
					match++;
				} else {

...
static struct DBParams pstable[] = {
	// tag    vlen  meta_dir       cm_conf_fn     th  pw   gen        range sec
	// nr
...
static void mixed_worker(const struct DBParams *const ps)
{
	...
	while (__ts.test_running) {
		...
		if (ps->p_writer > 0) {
			// 写入 100 kv 个
			const bool r =
				db_multi_insert(__ts.db, ps->p_writer, kvs);
			assert(r);
		}
		...
		for (uint64_t i = ps->p_writer; i < 100u; i++) {
			...
			struct KeyValue *const kv =
				db_lookup(__ts.db, sizeof(keys[i]),
		...
		if ((nr_100 % ps->nr_report) == 0) {
			pthread_mutex_lock((&__ts.test_lock));
			{
				...
				const double udiff =
					(usec - __ts.usec_last) / 1000000.0;
				const double elapsed =
					(usec - __ts.usec_start) / 1000000.0;
				const double qps =
					((double)ps->nr_report) * 100.0 / udiff;
...
static struct GenInfo *gen_initial(const char *const name, const uint64_t range)
{
...
static void mixed_test(const struct DBParams *const p)
{
	...
	for (uint64_t i = 0; i < nth; i++) {
		const int rc = pthread_create(&(pth[i]), &attr, mixed_thread,
					      (void *)p);
		assert(rc == 0);
		// 设置线程名
...
int main(int argc, char **argv)
{
	...
	char *dir2 = "/DataStruct_learn/Tree/lsm-trie/";
	// 字符串拼接
	memcpy(buf + cwd_len, dir2, strlen(dir2) + 1);
	// strcat(buf + cwd_len, dir2);

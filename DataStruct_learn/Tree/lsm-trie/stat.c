...
// 原子加1
void stat_inc(uint64_t *const p) { __sync_fetch_and_add(p, 1); }
...
void stat_show(struct Stat *const stat, FILE *const out)
{
    ...
    if (snapshot.nr_set)
    {
        ...
        fprintf(
            out, "nr_4K_write[table,0:4] %10lu %10lu %10lu %10lu %10lu %10lu\n",
...
void latency_95_99_999(uint32_t *const counters, FILE *const out)
{
    ...
    fprintf(out, "%s %6lu %s %6lu %s %6lu %s %6lu\n", "MAX",
            ...
            i99 * STAT_COUNTER_NUSEC, "99.9%", i999 * STAT_COUNTER_NUSEC);
}

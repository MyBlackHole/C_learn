				   ...
				   const uint64_t cap)
{
	...
	// 进行读或着写
	const ssize_t r = is_write ? pwrite(fd, buf, size, off) :
...
static void show_io(const char *const tag, const uint64_t io,
		    const uint64_t dur, const size_t mask, const uint64_t nr_th)
{
	...
	printf("[%s] #th %lu #IO %lu Unit %.0lf IOPS %0.2lf Vo %6.0lfMiB Thr "
	       "%.2lfMiB/s ",

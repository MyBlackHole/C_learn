...
static const struct MetaIndex *__find_metaindex(
    const uint64_t nr_mi, const struct MetaIndex *const mis, const uint16_t id)
{
    uint64_t lid = 0;
    uint64_t rid = nr_mi;
    while ((rid - lid) > 4)
    ...
    const uint16_t klen, const uint8_t *const key, const uint8_t *const hash)
{
    ...
    if ((kv == NULL) && (hash32 == mi->min) && (mi->id != mi->rid))
    {  // maybe in another barrel
        return metatable_recursive_lookup(mt, mi->rid, buf, klen, key, hash);
    }
    else
    {  // must in current barrel
        return kv;
    }
}

...
void BTree_split_child(BTNode *parent, int index, BTNode *node)
{
	...
	// 如果 node 不是叶子节点，拷贝 node 后半部分的指向孩子节点的指针，然后将nod
	// 后半部分指向孩子节点的指针置为NULL。
	if (!node->isLeaf) {
		for (i = 0; i < BTree_D; i++) {
...
// 将 node 中索引为 index + 1 的孩子节点合并到索引为 index
// 的孩子节点中，右孩子合并到左孩子结点中。 并调相关的 key
//
void BTree_merge_child(BTree *tree, BTNode *node, int index)
{
	...
	// 孩子节点的 keynum 必须均已达到下限，即均等于 BTree_D - 1
	assert(leftChild && leftChild->keynum == BTree_D - 1 && rightChild &&
	       rightChild->keynum == BTree_D - 1);

	// 将 node中关键字下标为index 的 key 下移至左孩子结点中，该key所对应
	// 右孩子结点指向node的右孩子结点中的第一个孩子。
	...
	for (i = 0; i < rightChild->keynum; ++i) {
		...
		leftChild->child[leftChild->keynum + 1] =
			rightChild->child[i + 1];
		++leftChild->keynum;
	}
  ...
  ============================================================*/
    ...
    ============================================================*/
		...
		if (child->keynum == BTree_D - 1) {
			...
			/*选择致富的相邻兄弟结点。*/
			// 3.1，如果所在孩子节点相邻的兄弟节点中有节点至少包含 BTree_D 个关键字
			// 将 node 的一个关键字key[index]下移到 child 中，将相对富有的相邻兄弟节点中一个关键
			// 上移到 node 中，然后在 child
			// 孩子节点中递归删除 key。
			// M
			if ((leftSibling && leftSibling->keynum >= BTree_D) ||
				...
				if (leftSibling && rightSibling) {
					richR = cmp(rightSibling->keynum,
						    leftSibling->keynum);
				}
				if (rightSibling &&
				    rightSibling->keynum >= BTree_D && richR) {
					// 相邻右兄弟相对富有，则左孩子先向父节点借一个元素，
					// 右兄弟中的第一个元素上移至父节点所借位置，并进行相应调整。
					child->key[child->keynum] =
						node->key[index];
					child->child[child->keynum + 1] =
						rightSibling->child[0];
					++child->keynum;

					node->key[index] = rightSibling->key[0];
					// 右兄弟元素前移
					for (j = 0;
					     j < rightSibling->keynum - 1;
					     ++j) {
						rightSibling->key[j] =
							rightSibling->key[j + 1];
						rightSibling->child[j] =
							rightSibling
								->child[j + 1];
					}
					rightSibling->key[rightSibling->keynum -
							  1] = 0;
					rightSibling
						->child[rightSibling->keynum -
							1] =
						rightSibling->child
							[rightSibling->keynum];
					rightSibling
					...
					// 相邻左兄弟相对富有，则该孩子向父节点借一个元素，
					// 左兄弟中的最后元素上移至父节点所借位置，并进行相应调整。
					for (j = child->keynum; j > 0;
					     --j) { //元素后移
						child->key[j] =
							child->key[j - 1];
						child->child[j + 1] =
					...
					leftSibling
						->key[leftSibling->keynum - 1] =
						0;
					leftSibling->child[leftSibling->keynum] =
						NULL;
				    ...
				    (leftSibling &&
				     leftSibling->keynum == BTree_D - 1)) &&
				   (!rightSibling ||
				    (rightSibling &&
				     rightSibling->keynum == BTree_D - 1))) {
				/*相邻兄弟结点都刚脱贫。删除前需要兄弟结点的合并操作,*/
				// 3b, 如果所在孩子节点相邻的兄弟节点都只包含 BTree_D - 1 个关键字，
				// 将 child 与其一相邻节点合并，并将 node
				// 中的一个关键字下降到合并节点中， 再在 node
				//  个关键字和相关指针，若 node 的 key 为空，删之，并调整根
				// 合并结点。
				// 最后，在相关孩子节点child中递归 除 key。
				if (leftSibling &&
				    leftSibling->keynum == BTree_D - 1) {
					// node中的右孩子元素合并到左孩子中。
					BTree_merge_child(tree, node,
							  index - 1);
...
//=====================================search====================================
// 1. 查找大于 tree->key[i] 小于 tree->key[i+1] 的 i 位置;
// 1.1 可能值是存在这样的 i 位置
// 1.2 遍历完 tree->keynum 也没找到
// 2. 查找等于 key 的 tree->key[i]
// 2.1 找到了，返回 tree 节点， 赋值 pos 为 i 代表在节点的哪个位置
// 2.2 没找到, 不是叶子节点, 获取叶子节点递归到 1 步
// 2.3 是叶子节点，没找到返回 NULL
...
#endif
		int pos = -1;
		//树的递归搜索,查看data[i]是否在树中，在树里，pos改为0。

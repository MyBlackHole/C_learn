...
size_t arraySearchValue(Array *array, void * const value)
{
    ...
    for (; index < array->len; ++index)
    {
        ...
        if (NULL != array->match)
        {
            cmp_n = array->match(begin_p + index * array->typeSize, value);
        }
        else
        {
            cmp_n = memcmp(begin_p + index * array->typeSize, value, array->typeSize);
        }
...
void arrayDelValue(Array *array, void *value)
{
    ...
    for (size_t i = 0; i < array->len; ++i)
    {
        if (!copy_p)
        {
            ...
            if (NULL != array->match)
            {
                cmp_n = array->match(begin_p + i * array->typeSize, value);
            }
            else
            {
                cmp_n = memcmp(begin_p + i * array->typeSize, value,
                              array->typeSize);
            }

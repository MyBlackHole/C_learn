...
int glob_recursive(const char *pattern, int flags, tb_list_ref_t list)
{
    ...
    if (info.type == TB_FILE_TYPE_DIRECTORY)
    {
        ...
        tb_strcat(path, "*");
        tb_trace_w("%s glob path:[%s] TB_FILE_TYPE_DIRECTORY", __FUNCTION__,
                   path);
    }
    ...
    for (index = 0; index < globres.gl_pathc; index++)
    {
        ...
        else if (info.type == TB_FILE_TYPE_FILE)
        {
            ...
            tb_char_t *path =
                tb_malloc0_cstr(tb_strlen(globres.gl_pathv[index]) + 1);
        ...
        {
            // 未知类型跳过
            tb_trace_w("%s glob path:[%s], info.type:[%d] ?????", __FUNCTION__,
                       globres.gl_pathv[index], info.type);
            continue;
        }
    }
    globfree(&globres);

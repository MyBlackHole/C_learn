...
// 模拟网络：模拟发送一个 udp包
auto udp_output(const char *buf, int len, ikcpcb *kcp, void *user) -> int
{
...
void test(int mode)
{
    ...
    // 最后一个是 user参数，用来传递标识
    ikcpcb *kcp1 = ikcp_create(0x11223344, (void *)nullptr);
    ...
    while (true)
    {
        ...
        {
            (reinterpret_cast<IUINT32 *>(buffer))[0] = index++;
            (reinterpret_cast<IUINT32 *>(buffer))[1] = current;
        ...
        while (true)
        {
            ...
            sumrtt += rtt;
            count++;
            if (rtt > static_cast<IUINT32>(maxrtt))
            {
                maxrtt = rtt;
            }

            printf("[RECV] mode=%d sn=%d rtt=%d\n", mode, static_cast<int>(sn),
                   static_cast<int>(rtt));
        }
        if (next > 1000)
        {
    ...
    printf("%s mode result (%dms):\n", names[mode], static_cast<int>(ts1));
    printf("avgrtt=%d maxrtt=%d tx=%d\n", static_cast<int>(sumrtt / count),
           maxrtt, vnet->tx1);

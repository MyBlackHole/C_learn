...
LatencySimulator *createLatencySimulator(int lostrate, int rttmin, int rttmax,
  ...
  latencySimulator->lostrate = lostrate / 2; // 上面数据是往返丢包率，单程除以2
...
void sendLatencySimulator(LatencySimulator *latencySimulator, int peer,
  ...
  if (peer == 0) {
    ...
    if (getRandom(latencySimulator->r12) < latencySimulator->lostrate)
      return;
    if ((int)tb_vector_size(latencySimulator->p12) >= latencySimulator->nmax)
      return;
  } else {
    latencySimulator->tx2++;
    if (getRandom(latencySimulator->r12) < latencySimulator->lostrate)
      return;
    if ((int)tb_vector_size(latencySimulator->p21) >= latencySimulator->nmax)
      return;
  }
  ...
  if (latencySimulator->rttmax > latencySimulator->rttmin)
    delay += rand() % (latencySimulator->rttmax - latencySimulator->rttmin);
  // 设置延迟时间

			       ...
			       SimplerpcContext *context)
{
	...
	rv->domain_to_service = g_hash_table_new_full(
		g_str_hash, g_str_equal, g_free, protobuf_c_server_destroy);
	gsk_stream_listener_handle_accept(listener, handle_accept,
					  handle_listener_error, rv, NULL);
}

SimplerpcServer *simplerpc_server_bind_ipv4(int tcp_port,
...
SimplerpcServer *simplerpc_server_bind_local(const char *path,
					     SimplerpcContext *context,
					     ProtobufCError **error)
{
	GskStreamListener *listener;
...
static void builtin_service_destroy(Simplerpc__Builtin_Service *ser)
{
...
SimplerpcContext *simplerpc_context_new(void)
{
	...
	context->domain_to_service = g_hash_table_new_full(
...
void simplerpc_context_add_service(SimplerpcContext *context,
				   const char *domain,
				   ProtobufCService *service)
{
...
SimplerpcClient *simplerpc_client_new_ipv4(const char *hostname, uint16_t port,
					   ProtobufCError **error){... }

ProtobufCService *simplerpc_client_new_service(
	SimplerpcClient *client, const char *domain,
	const ProtobufCServiceDescriptor *descriptor, ProtobufCError **error)
{
	...
}

...
static void word_closure_to_ptr_word(const Example__Word *message,
				     void *closure_data)
{
...
int main(int argc, char **argv)
{
	...
	if (service == NULL && addr == NULL)
		die("missing --addr=");
	if (service == NULL && addr != NULL) {
		channel = protobuf_c_sctp_channel_ipv4_connect_by_hostport(
			addr, dispatch);
		service = protobuf_c_sctp_channel_add_remote_service(
			channel, name, &example__word_funcs__descriptor);
	}
	...
	for (;;) {
		...
		if (strcmp(cmd, "uppercase") == 0 ||
			...
			input.word = at;
			if (strcmp(cmd, "uppercase") == 0)
				example__word_funcs__uppercase(
					service, &input,
					word_closure_to_ptr_word, &rv);
			else if (strcmp(cmd, "lowercase") == 0)
				example__word_funcs__lowercase(
					service, &input,
					word_closure_to_ptr_word, &rv);
			else
				assert(0);

	...
	while (fgets(buf, sizeof(buf), fp) != NULL) {
		...
		if (isspace(buf[0])) {
			...
			if (starts_with(start, "id "))
				...
				Foo__Person__PhoneNumber *pn = xmalloc(
				...
				tmp.type =
					start[0] == 'm' ?
						FOO__PERSON__PHONE_TYPE__MOBILE :
					start[0] == 'h' ?
						FOO__PERSON__PHONE_TYPE__HOME :
						FOO__PERSON__PHONE_TYPE__WORK;
				tmp.number = xstrdup(peek_next_token(start));
				person->phone = xrealloc(
					person->phone,
					sizeof(Foo__Person__PhoneNumber *) *
						(person->n_phone + 1));
				*pn = tmp;
...
static void example__by_name(Foo__DirLookup_Service *service,
...
int main(int argc, char **argv)
{
	...
	server = protobuf_c_rpc_server_new(
		address_type, name, (ProtobufCService *)&the_dir_lookup_service,
		NULL);

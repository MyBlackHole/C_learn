...
static void handle_query_response(const Foo__LookupResult *result,
				  void *closure_data)
{
	if (result == NULL)
...
int main(int argc, char **argv)
{
	...
	if (autoreconnect_millis >= 0)
		protobuf_c_rpc_client_set_autoreconnect_period(
	...
	for (;;) {
		...
		run_main_loop_without_blocking(
		...
		query.name = buf;
		foo__dir_lookup__by_name(service, &query, handle_query_response,
					 &is_done);
		while (!is_done)

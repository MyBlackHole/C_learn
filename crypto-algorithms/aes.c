...
void ccm_prepare_first_format_blk(BYTE buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len,
...
int aes_encrypt_ccm(const BYTE payload[], WORD payload_len, const BYTE assoc[], unsigned short assoc_len,
    ...
    increment_iv(temp_iv, AES_BLOCK_SIZE - 1 -
...
void aes_key_setup(const BYTE key[], WORD w[], int key_size) {
    int Nb = 4, Nr, Nk, idx;
    WORD temp, Rcon[] = {0x01000000, 0x02000000, 0x04000000,

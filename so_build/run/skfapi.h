...
#define USER_TYPE 0x00000001   // 用户PIN
    ...
    typedef struct Struct_RSAPRIVATEKEYBLOB
    {
        ...
        BYTE PrivateExponent
            [MAX_RSA_MODULUS_LEN];  // 私有密钥d，实际长度为BitLen/8字节
...
#pragma pack()
    ...
    ULONG DEVAPI SKF_Transmit(DEVHANDLE hDev, BYTE *pbCommand,
                              ULONG ulCommandLen, BYTE *pbData,
                              ULONG *pulDataLen);
     ...
     *	pulRetryCount	[OUT]管理员PIN码错误时，返回剩余重试次数
     */
    ULONG DEVAPI SKF_RemoteUnblockPIN(HAPPLICATION hApplication,
                                      BYTE *pbCipherPIN, ULONG ulCipherPINLen,
                                      ULONG *pulRetryCount);
    ...
    ULONG DEVAPI SKF_CreateApplication(
        DEVHANDLE hDev, LPSTR szAppName, LPSTR szAdminPin,
    ...
    */
     ...
     */
    ULONG DEVAPI SKF_ExtRSAEncrypt(DEVHANDLE hDev,
    ...
    ULONG DEVAPI SKF_GenerateAgreementDataWithECC(
        HCONTAINER hContainer, ULONG ulAlgId,
    ...
    */
     ...
     */
    ULONG DEVAPI SKF_DigestFinal(HANDLE hHash, BYTE *pHashData,
                                 ULONG *pulHashLen);
     ...
     */
    ULONG DEVAPI SKF_MacFinal(HANDLE hMac, BYTE *pbMacData,
                              ULONG *pulMacDataLen);
    /*
     *	关闭会话密钥、杂凑、消息认证码句柄。
     *	hHandle			[IN] 要关闭的对象句柄

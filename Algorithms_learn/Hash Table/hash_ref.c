...
HashMap *putInHeads(HashMap *hashMap, int index, Hash hash, Key key,
                    Value value) {
  ...
  (hashMap->usage)++;
  if (hashMap->usage > hashMap->realCapacity)
    return resize(hashMap);
  return hashMap;
}
HashMap *putInList(HashMap *hashMap, int index, Hash hash, Key key,
                   Value value) {

...
void Algo_8_16(Space *space) {
  ...
  /* 1.
   * 确定经过存储紧缩后，各内存块的新地址，换句话说，经过此步骤，可以生成一个新旧地址对照表
   */
  ...
  //    printf("日志1. 计算新
  //    地址对照表（至此，只完成了\"占用\"内存的前移，以及\"空闲\"内存的合并）：\n");
  //    PrintMemoryLayout(*space);
  ...
  //    printf("日志2. 用户变
  //    表更新完成（至此，用户变量(虚拟地址指针)已经指向了正确的内存，但是数据显示还不正常：\n");
  //    PrintMemoryLayout(*space);
  ...
  for (i = 1; i <= len; i++) {
    ...
    memmove(space->base + r->addr * BYTE, space->base + r->oldAddr * BYTE,
            r->size);
  }

...
public:
  enum class POLICY { RECURSIVE, NONRECURSIVE };
...
private:
  ...
  size_t nonrecursive(size_t steps) {
    ...
    if (result_.end() != iter) { // found.
      ...
      size_t start;
      for (start = steps; start != 2 and result_.end() == result_.find(start);
           --start) {
      }
...
int main() {
  ...
  std::cout << SolutionOFOneTwoStep()(5,
                                      SolutionOFOneTwoStep::POLICY::RECURSIVE)
            << std::endl;
  SolutionOFOneTwoStep::debug();

  std::cout << SolutionOFOneTwoStep()(
                   10, SolutionOFOneTwoStep::POLICY::NONRECURSIVE)
            << std::endl;
  SolutionOFOneTwoStep::debug();

  std::cout << SolutionOFOneTwoStep()(20,
                                      SolutionOFOneTwoStep::POLICY::RECURSIVE)
            << std::endl;
  SolutionOFOneTwoStep::debug();

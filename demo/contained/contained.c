...
/*
 * usage:sudo ./contained -m ~/workspace/aosp/Containers/minirootfs/ -u 0 -c
 * /bin/sh sudo ./contained -m /var/lib/lxc/ubuntu-focal/rootfs -u 0 -c /bin/sh
 */
#define _GNU_SOURCE
#include <errno.h>
#include <fcntl.h>
...
int resources(struct child_config *config)
{
    ...
    for (struct cgrp_control **cgrp = cgrps; *cgrp; cgrp++)
    {
             ...
             setting++)
        {
            ...
            int fd = 0;
            if (snprintf(path, sizeof(path), "%s/%s", dir, (*setting)->name) ==
                -1)
            {

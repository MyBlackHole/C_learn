...
定时时间到
*/
int MainTimerExpire(struct aeEventLoop *eventLoop, long long id,
		    void *clientData)
{
...
void MainReadFromClient(aeEventLoop *el, int fd, void *privdata, int mask)
{
	...
	if (nread <= 0) {
		...
		// 读数据正常，则把读到的数据返回给客户端，也就是客户端会收到发送的数据
...
void MainAcceptTcpHandler(aeEventLoop *el, int fd, void *privdata, int mask)
{
	int cfd, port;
	char ip_addr[128] = { 0 };
	cfd = anetTcpAccept(g_neterror_buf, fd, ip_addr, sizeof(ip_addr),
			    &port);
	...
	// 把accept返回的新套接字cfd注册到epoll事件集中，
	// 该新套接字关注AE_READABLE读事件，如果客户端有数据过来将会
	// 触发读回调函数MainReadFromClient
	if (aeCreateFileEvent(el, cfd, AE_READABLE, MainReadFromClient, NULL) ==
	    AE_ERR) {
...
int main()
{
	...
	// 某些情况下，服务端进程退出后，会有监听端口timwait状态的连接，
	// 这时候如果重启进程，会提示error:98，Address

...
// 拷贝 node_b index_b 用户数据到 node_a index_a 位置
...
static struct node *node_new(struct btree *btree, bool leaf)
{
	...
	size_t itemsoff = sz;
	// 计算 max_items 个用户数据一共占用内存加上 node * max_items 个内存大小
...
static bool grow_group(struct btree *btree, struct group *group)
{
	...
	// 拷贝 group->len 个 node * 的 group->nodes 内容到 nodes
...
{
	memcpy(left->items + elsize * (size_t)left->num_items, right->items,
			...
			void *udata)
{
	return btree_new_with_allocator(
		...
		(_free ? _free : free), elsize, max_items, compare, udata);
}
		       ...
		       struct node **right, void **median, bool lean_left)
{
	...
	if (lean_left) {
		...
		int mdif =
			(node->num_items - (mid + 1)) - (int)btree->min_items;
		if (mdif < 0) {
			mid += mdif;
		}
	} else {
		     ...
		     bool *found, uint64_t *hint, int depth)
{
	...
	if (hint && depth < 8) {
		...
		if (index > 0) {
			...
			void *item =
				get_item_at(btree->elsize, node, (size_t)index);
		     ...
		     bool lean_left, uint64_t *hint, int depth)
{
	...
	if (found) {
		// 已存在
		swap_item_at(btree->elsize, node, (size_t)i, item,
			     btree->spares[0]);
		return true;
	}
	...
	// 递归往下找
	if (node_set(btree, node->children[i], item, lean_left, hint,
		     depth + 1)) {
		return true;
	}
			...
			int depth)
{
	...
	if (found) {
		if (act == POPMAX) {
			...
			set_item_at(btree->elsize, node, (size_t)i,
				    btree->spares[2]);
			deleted = true;
		}
	} else {
		      ...
		      bool (*iter)(const void *item, void *udata), void *udata)
{
	if (node->leaf) {
		for (int i = 0; i < node->num_items; i++) {
			if (!iter(get_item_at(btree->elsize, node, (size_t)i),
				  udata)) {
				return false;
			}
		}
...
static int
node_action_ascend(struct btree *btree, struct node *node, void **pivot,
		   ...
		   void *udata, uint64_t *hint, int depth)
{
	...
	for (; i < node->num_items; i++) {
		if (!node->leaf) {
			int ret = node_action_ascend(btree, node->children[i],
						     pivot, iter, udata, hint,
						     depth + 1);
			if (ret != BTCONTINUE) {
				return ret;
			}
		}
		copy_item_into(btree->elsize, node, (size_t)i,
			       btree->spares[0]);
		...
		case BTREE_DELETE:
			if (node->leaf &&
			    (size_t)node->num_items > btree->min_items) {
				// delete in place
				node_shift_left(btree->elsize, node, (size_t)i,
						false);
				btree->count--;
				i--;
				break;
		...
		case BTREE_UPDATE: {
			void *item =
				get_item_at(btree->elsize, node, (size_t)i);
			if (btcompare(btree, item, btree->spares[0])) {
				// Item keys have diverged. This is not fatal, but we need
				// to retry the operation until we get the response we're
				// looking for. There is a risk that a user, who does not
				// understand that the updated item must match exactly with
				// the previous item (ie "compare(a, b) == 0") , might
				// create an infinite loop like scenario.
				i--;
			} else {
	...
	if (!node->leaf) {
		int ret = node_action_ascend(btree, node->children[i], pivot,
					     iter, udata, hint, depth + 1);
		if (ret != BTCONTINUE) {
			return ret;
		}
	}
...
static int
node_action_descend(struct btree *btree, struct node *node, void **pivot,
		    enum btree_action (*iter)(void *item, void *udata),
		    void *udata, uint64_t *hint, int depth)
{
	bool found = false;
	...
	if (!node->leaf && !found) {
		int ret = node_action_descend(btree, node->children[i], pivot,
					      iter, udata, hint, depth + 1);
		if (ret != BTCONTINUE) {
			return ret;
		}
	...
	for (; i >= 0; i--) {
		...
		if (!node->leaf) {
			int ret = node_action_descend(btree, node->children[i],
						      pivot, iter, udata, hint,
						      depth + 1);
			if (ret != BTCONTINUE) {
				return ret;
			      ...
			      void *udata, uint64_t *hint)
{
	reset_load_fields(btree);
	while (btree->root) {
		int ret = node_action_ascend(btree, btree->root, &pivot, iter,
					     udata, hint, 0);
		if (ret != BTSTARTOVER) {
			break;
		}
			       ...
			       void *udata, uint64_t *hint)
{
	reset_load_fields(btree);
	while (btree->root) {
		int ret = node_action_descend(btree, btree->root, &pivot, iter,
					      udata, hint, 0);
		if (ret != BTSTARTOVER) {
			break;
		}
		       ...
		       void (*print)(void *), int depth)
{
	if (node->leaf) {
		...
		node_print(btree, node->children[node->num_items], print,
			   depth + 1);
	}
}
...
static bool node_saneheight(struct node *node, int height, int maxheight)
{
	if (node->leaf) {
		...
		int i = 0;
		for (; i < node->num_items; i++) {
			if (!node_saneheight(node->children[i], height + 1,
					     maxheight)) {
				return false;
			}
		}
...
static bool node_saneprops(struct btree *btree, struct node *node, int height)
{
	...
	if (!node->leaf) {
		for (int i = 0; i < node->num_items; i++) {
			if (!node_saneprops(btree, node->children[i],
					    height + 1)) {
				return false;
			}
		}
...
{
	int max_items = getenv("MAX_ITEMS") ? atoi(getenv("MAX_ITEMS")) :
					      def_MAX_ITEMS;
	...
	for (int i = 2; i < 16; i++) {
		memset(&ctx, 0, sizeof(struct pair_keep_ctx));
		btree_action_descend(btree, &pairs[N / i], pair_keep_desc,
				     &ctx);
...
static void test_basic()
{
	...
	printf("== testing ascend\n");
	{
		...
		for (int i = 0; i < N; i++) {
			struct iter_ctx ctx = { .btree = btree, .rev = false };
			bool ret = btree_ascend(btree, &(int){ i * 10 }, iter,
						&ctx);
		...
		for (int i = 0; i < N; i++) {
			struct iter_ctx ctx = { .btree = btree, .rev = false };
			bool ret = btree_ascend(btree, &(int){ i * 10 - 1 },
						iter, &ctx);
			assert(ret && !ctx.bad && ctx.count == N - i);
		}
	...
	printf("== testing descend\n");
	{
		...
		for (int i = N - 1, j = 0; i >= 0; i--, j++) {
			struct iter_ctx ctx = { .btree = btree, .rev = true };
			bool ret = btree_descend(btree, &(int){ i * 10 }, iter,
						 &ctx);
		...
		for (int i = N - 1; i >= 0; i--) {
			struct iter_ctx ctx = { .btree = btree, .rev = true };
			bool ret = btree_descend(btree, &(int){ i * 10 + 1 },
						 iter, &ctx);
			assert(ret && !ctx.bad && ctx.count == N - (N - i) + 1);
		}
...
#define bench(name, N, code)                                              \
	...
	if (ns_op < 10) {                                                 \
		printf("%d ops in %.3f secs, %.1f ns/op, %.0f op/sec", N, \
		       elapsed_secs, ns_op, (double)N / elapsed_secs);    \
	} else {                                                          \
...
static void benchmarks()
{
	...
	bench("load (seq)", N, { btree_load(btree, &vals[i]); })
		btree_free(btree);

	shuffle(vals, N, sizeof(int));
	btree = btree_new(sizeof(int), max_items, compare_ints, nothing);
	bench("load (rand)", N, { btree_set_hint(btree, &vals[i], &hint); })
		btree_free(btree);

	btree = btree_new(sizeof(int), max_items, compare_ints, nothing);
	qsort(vals, N, sizeof(int), compare_ints_nudata);
	bench("set (seq)", N, { btree_set(btree, &vals[i]); })
		btree_free(btree);

	////
	qsort(vals, N, sizeof(int), compare_ints_nudata);
	btree = btree_new(sizeof(int), max_items, compare_ints, nothing);
	bench("set (seq-hint)", N, { btree_set_hint(btree, &vals[i], &hint); })
		btree_free(btree);

	////
	shuffle(vals, N, sizeof(int));
	btree = btree_new(sizeof(int), max_items, compare_ints, nothing);
	bench("set (rand)", N, { btree_set(btree, &vals[i]); })

		qsort(vals, N, sizeof(int), compare_ints_nudata);
	bench("get (seq)", N, { btree_get(btree, &vals[i]); })

		bench("get (seq-hint)", N,
		      { btree_get_hint(btree, &vals[i], &hint); })

			shuffle(vals, N, sizeof(int));
	bench("get (rand)", N, { btree_get(btree, &vals[i]); })

		shuffle(vals, N, sizeof(int));
	bench("delete (rand)", N, { btree_delete(btree, &vals[i]); })
		shuffle(vals, N, sizeof(int));
	for (int i = 0; i < N; i++) {
		btree_set(btree, &vals[i]);
	}

	bench("min", N, { assert(btree_min(btree)); })

		bench("max", N, { assert(btree_max(btree)); })

			bench("ascend", N,
			      {
				      btree_ascend(btree, NULL, simple_iter,
						   NULL);
				      break;
			      })
	...
	bench("pop-max", N, { btree_pop_max(btree); })

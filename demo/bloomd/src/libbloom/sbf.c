...
int sbf_from_filters(bloom_sbf_params *params, bloom_sbf_callback cb,
                     void *cb_in, uint32_t num_filters,
                     bloom_bloomfilter **filters, bloom_sbf *sbf) {
  // Copy the params
...
static int sbf_append_filter(bloom_sbf *sbf) {
  ...
  double fp_prob = sbf_inital_probability(sbf->params.fp_probability,
                                          sbf->params.probability_reduction);
  ...
  if (sbf->num_filters > 1) {
    memcpy(sbf->filters + 1, old_filters,
           (sbf->num_filters - 1) * sizeof(bloom_bloomfilter *));
    memcpy(sbf->dirty_filters + 1, old_dirty,
           (sbf->num_filters - 1) * sizeof(unsigned char));
    memcpy(sbf->capacities + 1, old_capacities,
           (sbf->num_filters - 1) * sizeof(uint64_t));
    free(old_filters);
    free(old_dirty);
    free(old_capacities);
  }
...
static void sbf_init_capacities(bloom_sbf *sbf) {
  ...
  for (uint32_t i = 0; i < sbf->num_filters; i++) {
    ...
    capacity =
        init_capacity * pow(sbf->params.scale_size, (sbf->num_filters - i - 1));

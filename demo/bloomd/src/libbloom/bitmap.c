...
static int flush_page(bloom_bitmap *map, uint64_t page, uint64_t size,
                      uint64_t max_page);
 ...
 */
int bitmap_from_file(int fileno, uint64_t len, bitmap_mode mode,
                     bloom_bitmap *map) {
  ...
  // Perform the map in
  unsigned char *addr = mmap(NULL, len, PROT_READ | PROT_WRITE, flags,
                             ((mode == PERSISTENT) ? -1 : newfileno), 0);
 ...
 * @return 0 on success. Negative on error.
 */
int bitmap_from_filename(char *filename, uint64_t len, int create,
                         bitmap_mode mode, bloom_bitmap *map) {
  // Get the flags
  // 配置标识
  int flags = O_RDWR;
  if (create) {
  ...
  if (create) {
    ...
    if ((uint64_t)buf.st_size == 0) {
      ...
      // 文件存在但是大小不对
      syslog(LOG_ERR,
             "File size does not match length but is already truncated!");
      close(fileno);
      return -1;
    }
  }

...
/*
bloom_bitmap *bitmap_from_filename(char* filename, size_t len, int create, int
resize) {
*/
...
START_TEST(make_bitmap_nofile_create) {
  bloom_bitmap map;
  int res =
      bitmap_from_filename("/tmp/mmap_nofile_create", 4096, 1, SHARED, &map);
...
START_TEST(make_bitmap_nofile_create_persistent) {
  bloom_bitmap map;
  int res = bitmap_from_filename("/tmp/mmap_nofile_create_persist", 4096, 1,
                                 PERSISTENT, &map);
...
START_TEST(flush_bitmap_file) {
  bloom_bitmap map;
  int res =
      bitmap_from_filename("/tmp/mmap_flush_bitmap", 8196, 1, SHARED, &map);
  fail_unless(res == 0);
  fail_unless(bitmap_flush(&map) == 0);
...
START_TEST(flush_bitmap_file_persistent) {
  bloom_bitmap map;
  int res = bitmap_from_filename("/tmp/mmap_flush_bitmap_persist", 8196, 1,
                                 PERSISTENT, &map);
  fail_unless(res == 0);
  fail_unless(bitmap_flush(&map) == 0);
...
START_TEST(flush_bitmap_null) { fail_unless(bitmap_flush(NULL) == -EINVAL); }
END_TEST
...
START_TEST(close_bitmap_file) {
  bloom_bitmap map;
  int res =
      bitmap_from_filename("/tmp/mmap_close_bitmap", 8196, 1, SHARED, &map);
  fail_unless(res == 0);
  fail_unless(bitmap_close(&map) == 0);
  fail_unless(map.mmap == NULL);
...
START_TEST(close_bitmap_null) { fail_unless(bitmap_close(NULL) == -EINVAL); }
END_TEST
...
START_TEST(getbit_bitmap_file_zero) {
  bloom_bitmap map;
  int res =
      bitmap_from_filename("/tmp/mmap_getbit_zero", 4096, 1, SHARED, &map);
  fail_unless(res == 0);
  for (int idx = 0; idx < 4096 * 8; idx++) {
    fail_unless(bitmap_getbit((&map), idx) == 0);
  }
...
START_TEST(getbit_bitmap_file_persist_zero) {
  bloom_bitmap map;
  int res = bitmap_from_filename("/tmp/persist_getbit_zero", 4096, 1,
                                 PERSISTENT, &map);
  fail_unless(res == 0);
  for (int idx = 0; idx < 4096 * 8; idx++) {
    fail_unless(bitmap_getbit((&map), idx) == 0);
...
START_TEST(getbit_bitmap_file_persist_one) {
  bloom_bitmap map;
  int res = bitmap_from_filename("/tmp/persist_getbit_one", 4096, 1, PERSISTENT,
                                 &map);
  fail_unless(res == 0);
  memset(map.mmap, 255, 4096);
  for (int idx = 0; idx < 4096 * 8; idx++) {
...
START_TEST(setbit_bitmap_file_persist_one) {
  bloom_bitmap map;
  int res = bitmap_from_filename("/tmp/persist_setbit_one", 4096, 1, PERSISTENT,
                                 &map);
  fail_unless(res == 0);
  for (int idx = 0; idx < 4096 * 8; idx++) {
    bitmap_setbit((&map), idx);
...
START_TEST(flush_does_write) {
  bloom_bitmap map;
  int res =
      bitmap_from_filename("/tmp/mmap_flush_write", 4096, 1, SHARED, &map);
...
START_TEST(close_does_flush) {
  bloom_bitmap map;
  int res =
      bitmap_from_filename("/tmp/mmap_close_flush", 4096, 1, SHARED, &map);
  fchmod(map.fileno, 0777);
  fail_unless(res == 0);
  for (int idx = 0; idx < 4096 * 8; idx++) {
    bitmap_setbit((&map), idx);
...
START_TEST(flush_does_write_persist) {
  bloom_bitmap map;
  int res = bitmap_from_filename("/tmp/persist_flush_write", 4096, 1,
                                 PERSISTENT, &map);
  fchmod(map.fileno, 0777);
  fail_unless(res == 0);
  for (int idx = 0; idx < 4096 * 8; idx++) {
  ...
  bloom_bitmap map2;
  res = bitmap_from_filename("/tmp/persist_flush_write", 4096, 0, PERSISTENT,
                             &map2);
  fail_unless(res == 0);
  for (int idx = 0; idx < 4096; idx++) {
    fail_unless(map2.mmap[idx] == 255);
...
START_TEST(close_does_flush_persist) {
  ...
  res = bitmap_from_filename("/tmp/persist_close_flush", 4096, 0, PERSISTENT,
                             &map);
  fail_unless(res == 0);
  for (int idx = 0; idx < 4096; idx++) {
    fail_unless(map.mmap[idx] == 255);

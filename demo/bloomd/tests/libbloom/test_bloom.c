...
START_TEST(test_flush_close) {
  ...
  bloom_bitmap map;
  bloom_bloomfilter filter;
  bitmap_from_filename("/tmp/test_flush_close.mmap", params.bytes, 1, SHARED,
                       &map);
  bf_from_bitmap(&map, params.k_num, 1, &filter);
...
START_TEST(test_bf_flush) {
  ...
  bloom_bitmap map;
  bloom_bloomfilter filter;
  fail_unless(bitmap_from_filename("/tmp/test_flush.mmap", params.bytes, 1,
                                   SHARED, &map) == 0);
...
START_TEST(test_bf_close_does_flush) {
  ...
  bloom_bitmap map;
  bloom_bloomfilter filter;
  fail_unless(bitmap_from_filename("/tmp/test_close_does_flush.mmap",
                                   params.bytes, 1, SHARED, &map) == 0);
  fail_unless(bf_from_bitmap(&map, params.k_num, 1, &filter) == 0);
  fchmod(map.fileno, 0777);
...
START_TEST(test_bf_shared_compatible_persist) {
  ...
  bloom_bitmap map;
  bloom_bloomfilter filter;
  fail_unless(bitmap_from_filename("/tmp/shared_compat_persist.mmap",
                                   params.bytes, 1, PERSISTENT, &map) == 0);
  fail_unless(bf_from_bitmap(&map, params.k_num, 1, &filter) == 0);
  fchmod(map.fileno, 0777);

	...
	while (again) {
		...
		default:
			...
			if (!ret || getchar() != '\n') {
				...
				fprintf(stderr,
					"Block size must be positive and pow of 2!\n");
				again = 1;
			} else if (size <= 0 || (size & (size - 1)) != 0) {
				again = 1;
			} else {
				int order = (size - sizeof(struct bplus_node)) /
					    (sizeof(key_t) + sizeof(off_t));
				if (size < (int)sizeof(struct bplus_node) ||
				    order <= 2) {
					fprintf(stderr,
						"block size is too small for one node!\n");
					again = 1;
				} else {

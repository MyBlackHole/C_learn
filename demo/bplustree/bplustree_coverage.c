	...
	for (;;) {
		...
		case 'i':
			...
			if (0 == bplus_tree_put(tree, k, k)) {
				...
				if (exist) {
					fprintf(stderr,
						"insert key %d error, has been inserted before!\n",
						k);
					exit(-1);
				}
			} else {
				if (!exist) {
					fprintf(stderr,
						"insert key %d error, could be inserted!\n",
						k);
					exit(-1);
				}
			}
		...
		case 's':
			...
			if (ret != -1) {
				...
				if (!exist) {
					fprintf(stderr,
						"search key %d error, found but not exists!\n",
						k);
					exit(-1);
				}
				assert(ret == k);
				...
				if (exist) {
					fprintf(stderr,
						"search key %d error, exists but not found!\n",
						k);
					exit(-1);
				}
			}
		...
		case 'r':
			...
			if (0 == bplus_tree_put(tree, k, 0)) {
				...
				if (!exist) {
					fprintf(stderr,
						"delete key %d error, deleted but not found!\n",
						k);
					exit(-1);
				}
			} else {
				if (exist) {
					fprintf(stderr,
						"delete key %d error, found but not deleted!\n",
						k);
					exit(-1);
				}
			}

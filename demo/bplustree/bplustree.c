...
static key_t non_leaf_split_left(struct bplus_tree *tree,
			   ...
			   key_t key)
{
	...
	if (node->children == _max_order) {
		...
		if (insert < split) {
			split_key = non_leaf_split_left(
				tree, node, sibling, l_ch, r_ch, key, insert);
		} else if (insert == split) {
			split_key = non_leaf_split_right1(
				tree, node, sibling, l_ch, r_ch, key, insert);
		} else {
			split_key = non_leaf_split_right2(
				tree, node, sibling, l_ch, r_ch, key, insert);
		}

		/* build new parent */
		if (insert < split) {
			return parent_node_build(tree, sibling, node,
						 split_key);
		} else {
			return parent_node_build(tree, node, sibling,
						 split_key);
		}
	} else {
		       ...
		       key_t key, long data)
{
	...
	if (leaf->children == _max_entries) {
		...
		/* build new parent */
		if (insert < split) {
			return parent_node_build(tree, sibling, leaf,
						 split_key);
		} else {
			return parent_node_build(tree, leaf, sibling,
						 split_key);
		}
	} else {
				     ...
				     int parent_key_index, int remove)
{
	...
	memmove(&key(left)[left->children], &key(node)[0],
		remove * sizeof(key_t));
	...
	if (node->parent == INVALID_OFFSET) {
		...
		if (sibling_select(l_sib, r_sib, parent, i) == LEFT_SIBLING) {
			if (l_sib->children > (_max_order + 1) / 2) {
				non_leaf_shift_from_left(tree, node, l_sib,
							 parent, i, remove);
				/* flush nodes */
				node_flush(tree, node);
				...
				non_leaf_merge_into_left(tree, node, l_sib,
							 parent, i, remove);
			...
			if (r_sib->children > (_max_order + 1) / 2) {
				non_leaf_shift_from_right(tree, node, r_sib,
							  parent, i + 1);
				/* flush nodes */
				node_flush(tree, node);
				node_flush(tree, l_sib);
				...
				non_leaf_merge_from_right(tree, node, r_sib,
							  parent, i + 1);
				/* delete empty right sibling and flush */
				struct bplus_node *rr_sib =
					node_fetch(tree, r_sib->next);
				 ...
				 int remove)
{
	...
	memmove(&key(left)[left->children], &key(leaf)[0],
		remove * sizeof(key_t));
	...
	if (leaf->parent == INVALID_OFFSET) {
		...
		if (sibling_select(l_sib, r_sib, parent, i) == LEFT_SIBLING) {
			if (l_sib->children > (_max_entries + 1) / 2) {
				leaf_shift_from_left(tree, leaf, l_sib, parent,
						     i, remove);
				/* flush leaves */
				node_flush(tree, leaf);
				node_flush(tree, l_sib);
				...
				leaf_merge_into_left(tree, leaf, l_sib, i,
						     remove);
			...
			if (r_sib->children > (_max_entries + 1) / 2) {
				leaf_shift_from_right(tree, leaf, r_sib, parent,
						      i + 1);
				/* flush leaves */
				node_flush(tree, leaf);
				node_flush(tree, l_sib);
...
struct bplus_tree *bplus_tree_init(char *filename, int block_size)
{
	...
	_max_order =
		(block_size - sizeof(node)) / (sizeof(key_t) + sizeof(off_t));
	_max_entries =
		(block_size - sizeof(node)) / (sizeof(key_t) + sizeof(long));
	if (_max_order <= 2) {
		fprintf(stderr, "block size is too small for one node!\n");
		return NULL;
	}
	...
	_max_order =
		(_block_size - sizeof(node)) / (sizeof(key_t) + sizeof(off_t));
	_max_entries =
...
void bplus_tree_deinit(struct bplus_tree *tree)
{
	...
	list_for_each_safe(pos, n, &tree->free_blocks) {
		list_del(pos);
		struct free_block *block =
			list_entry(pos, struct free_block, link);
...
void bplus_tree_dump(struct bplus_tree *tree)
{
	...
	for (;;) {
		if (node != NULL) {
			...
			node = is_leaf(node) ?
				       NULL :
				       node_seek(tree, sub(node)[sub_idx]);
		} else {

...
int main(void)
{
    ...
    // demonstrate. There are a few important notes before we start:
    ...
    // That's all the information and preparation LZ4 needs to compress *src
    // into* compressed_data. Invoke LZ4_compress_default now with our size
    // values and pointers to our memory locations. Save the return value for
    // error checking.
    ...
    if (compressed_data_size <= 0)
        run_screaming(
            "A 0 or negative result from LZ4_compress_default() "
    ...
    // the number of bytes required. You can use this to realloc()
    // *compress_data to free up memory, if desired.  We'll do so just to
    // demonstrate the concept.
    compressed_data =
        (char *)realloc(compressed_data, (size_t)compressed_data_size);
    if (compressed_data == NULL)
        run_screaming("Failed to re-alloc memory for compressed_data.  Sad :(",
                      1);
    ...
    // this example, we just re-use this information from previous section, but
    // in a real-world scenario, metadata must be transmitted to the
    // decompression side. Each implementation is in charge of this part.
    // Oftentimes, it adds some header of its own. Sometimes, the metadata can
    // be extracted from the local context.

    // First, let's create a *new_src location of size src_size since we know
    // that value.
    ...
    // value returned == number of bytes regenerated from compressed_data
    // stream.

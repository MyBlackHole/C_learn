...
int demo_file_compress_main(int argc, const char **argv)
{
    ...
    /* compress */
    {
        ...
        printf("%s: %zu → %zu bytes, %.1f%%\n", inpFilename,
               ...
               get_file_size(lz4Filename), /* might overflow is size_t is 32
                                              bits and size_{in,out} > 4 GB */
               (double)get_file_size(lz4Filename) / get_file_size(inpFilename) *
                   100);
    ...
    /* verify */
    {
        ...
        {
            printf(
                "corruption detected : decompressed file differs from "
                "original\n");
            return cmp;
        }
